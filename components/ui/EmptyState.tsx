...
const DefaultIcon = () => (
  <svg
    className="w-10 h-10"style={{ color: "var(--rtm-border)"}}
...
export default function EmptyState({
  ...
  const containerClass =
    variant === "table"? "py-10 px-4": variant === "page"? "py-24 px-4": "py-16 px-4";

  return (
    <div className={`flex flex-col items-center justify-center text-center ${containerClass}`}>
      <div
        className="w-16 h-16 rounded-2xl flex items-center justify-center mb-4"style={{ background: "var(--rtm-bg)", border: "1px solid var(--rtm-border)"}}
      >
        {icon ?? <DefaultIcon />}
      </div>
