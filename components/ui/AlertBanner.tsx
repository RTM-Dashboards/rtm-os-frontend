    ...
    <div className="space-y-2">
      {alerts.map((alert) => {
        ...
        return (
          <div
            key={alert.id}
            className="flex items-start gap-3 p-3.5 rounded-xl border"style={{ background: s.bg, borderColor: s.border }}
          >
            {/* Icon circle */}
            <span
              className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5"style={{ background: s.iconBg, color: s.textColor }}
            >
            ...
            {alert.action && (
              <button
                className="text-xs font-semibold flex-shrink-0 px-2.5 py-1 rounded-lg border transition-opacity hover:opacity-80"style={{ color: s.titleColor, borderColor: s.border, background: s.iconBg }}
              >
                {alert.action}
              </button>
            )}
