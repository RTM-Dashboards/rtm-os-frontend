    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p
            className="text-xs font-semibold uppercase tracking-wide truncate"style={{ color: "var(--rtm-text-muted)"}}
          >
            {title}
          </p>
          <p
            className="mt-1.5 text-2xl font-bold tracking-tight"style={{ color: "var(--rtm-text-primary)"}}
          >
            {value}
          </p>
          {subtitle && (
        ...
        {icon && (
          <div
            className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0"style={{ background: iconBg, color: iconColor }}
          >
            {icon}
          </div>
        )}
      ...
      {(trendValue && ts) || rs ? (
        <div className="flex items-center gap-2">
          ...
          {rs ? (
            <span
              className="inline-flex items-center gap-1.5 text-xs font-semibold px-2 py-0.5 rounded-full border"style={{ background: rs.bg, color: rs.color, borderColor: rs.border }}
            >
            ...
            <>
              <span
                className="inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full border"style={{ background: ts.bg, color: ts.color, borderColor: ts.border }}
              >
