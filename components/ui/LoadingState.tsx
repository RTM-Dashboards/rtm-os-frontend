  ...
  return (
    <div className="w-full space-y-3 animate-pulse">
      {[0.75, 0.5, 0.833, 0.667].map((w, i) => (
        <div
          key={i}
          className="h-4 rounded"style={{ width: `${w * 100}%`, background: "var(--rtm-border)"}}
        />
      ))}
    </div>
  );
  ...
  return (
    <div className="flex items-center gap-1.5">
      {[0, 1, 2].map((i) => (
        <span
          key={i}
          className="w-2 h-2 rounded-full animate-bounce"style={{ background: "var(--rtm-blue)", animationDelay: `${i * 150}ms` }}
        />
      ))}
    </div>
  );
