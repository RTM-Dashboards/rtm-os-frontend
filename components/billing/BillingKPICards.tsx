...
function KPIItem({ title, value, subtitle, trend, trendValue, icon, iconBg }: KPIItemProps) {
  const trendStyle =
    trend === "up"? { bg: "#ECFDF5", color: "#059669", border: "#A7F3D0"}
      : trend === "down"? { bg: "#FEF2F2", color: "#DC2626", border: "#FECACA"}
  ...
  return (
    <div
      className="rounded-xl border p-5 flex flex-col gap-4 hover:shadow-md transition-shadow"style={{
    ...
    >
      ...
      {trendValue && trend && (
        <div className="flex items-center gap-2">
          <span
            className="inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full border"style={{ background: trendStyle.bg, color: trendStyle.color, borderColor: trendStyle.border }}
          >
