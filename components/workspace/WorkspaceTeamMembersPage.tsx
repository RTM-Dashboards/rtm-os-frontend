    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        {summaryChips.map(({ label, value, bg, color }) => (
          <div
            key={label}
            className="flex items-center gap-2 rounded-xl px-4 py-2.5 border"style={{
              background: bg,
              color,
      ...
      {/* ── Team table ── */}
      <SectionWrapper
        title="All Members"description={`${workspace.name} team roster`}
        noPadding
        actions={
          rolesRoute ? (
            <Link
              href={rolesRoute}
              className="inline-flex items-center gap-1 text-xs font-semibold px-3 py-1.5 rounded-lg border transition-colors"style={{
      ...
      {/* ── Role breakdown ── */}
      <SectionWrapper
        title="Role Breakdown"description="Member distribution by role title">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {Array.from(roleMap.entries()).map(([role, count]) => (
            <div
              key={role}
              className="flex flex-col gap-1 p-3 rounded-lg"style={{
                background: "var(--rtm-bg)",
                border: "1px solid var(--rtm-border-light)",
              }}
            >
              <span
                className="text-2xl font-bold"style={{ color: workspace.accentColor }}
              >
                {count}
              </span>
              <span
                className="text-[11px] font-medium leading-snug"style={{ color: "var(--rtm-text-secondary)"}}
              >
                {role}
              </span>
            </div>
      ...
      <div className="flex flex-wrap gap-2">
        <Link
          ...
          className="inline-flex items-center gap-1 rounded-lg border px-4 py-2 text-sm font-medium transition-colors"style={{
