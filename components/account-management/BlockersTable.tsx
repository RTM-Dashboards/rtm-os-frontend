...
export function BlockersTable({ blockers }: BlockersTableProps) {
  ...
  const openCount = blockers.filter(
    (b) => b.status === "Open"|| b.status === "Awaiting Client"|| b.status === "In Review").length;

  return (
    <SectionCard
      title="Blockers"subtitle="Items blocking progress"badge={openCount > 0 ? `${openCount} active` : "0"}
    ...
    >
      <div className="px-5 py-3 border-b border-slate-100">
        <SearchFilter
          ...
          placeholder="Search blockers..."resultCount={filtered.length}
          filters={[
            {
              label: "Status",
              value: filterStatus,
                ...
                <td className="px-4 py-3">
                  <span
                    className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${
                      b.blockedBy === "Client"? "bg-[var(--rtm-blue-light)] text-orange-700": "bg-purple-100 text-purple-700"}`}
                  >
                    {b.blockedBy}
                  </span>
                </td>
