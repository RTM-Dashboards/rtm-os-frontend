...
export default function RenewalReportPanel({ renewals }: Props) {
  ...
  return (
    <SectionWrapper
      title="Renewal Pipeline"description="Upcoming contract renewals requiring proactive AM action"actions={
      ...
      <div className="space-y-3">
        {renewals.map((r, idx) => {
          ...
          return (
            <div
              key={idx}
              className="rounded-xl border p-4"style={{ background: "var(--rtm-bg)", borderColor: "var(--rtm-border-light)"}}
            >
              <div className="flex items-start justify-between gap-3 mb-2">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span
                    className="text-[11px] font-bold px-2 py-0.5 rounded-full border"style={{ background: risk.bg, color: risk.color, borderColor: risk.border }}
                  >
                    {risk.label}
                  </span>
