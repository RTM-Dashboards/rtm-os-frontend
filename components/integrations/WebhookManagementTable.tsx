...
export default function WebhookManagementTable({ webhooks }: Props) {
  return (
    <SectionWrapper
      title="Webhook Management"description="Inbound and outbound webhooks across all configured integrations"actions={
        <button
      ...
      <div className="space-y-3">
        {webhooks.map((webhook) => {
          ...
          return (
            <div
              key={webhook.id}
              className="rounded-xl border p-4"style={{ borderColor: "var(--rtm-border)", background: "var(--rtm-surface)"}}
            >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div>
                  ...
                  <p
                    className="text-xs font-semibold"style={{ color: webhook.errorCount > 0 ? "#DC2626": "#059669"}}
                  >
                    {webhook.errorCount}
                  </p>
                </div>
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {webhook.events.map((event) => (
                      <span
                        key={event}
                        className="text-[10px] font-mono px-1.5 py-0.5 rounded"style={{
