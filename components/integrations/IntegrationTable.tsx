...
export default function IntegrationTable({ integrations, onSelect }: Props) {
  ...
  return (
    <SectionWrapper
      ...
      description={`${filtered.length} of ${integrations.length} connectors shown`}
      actions={
        <button className="px-3 py-1.5 rounded-lg text-xs font-semibold text-white transition-colors"
                      ...
                      {integration.departmentsUsing.length === 0 ? (
                        ...
                        integration.departmentsUsing.slice(0, 2).map((d) => (
                          <span
                            ...
                            style={{ background: "var(--rtm-border-light)", color: "var(--rtm-text-secondary, #475569)" }}
                          >
                            {d}
                          </span>
                        ))
