...
export default function CallIntelligencePanel({ mappings, integrations }: Props) {
  return (
    <SectionWrapper
      title="Call Intelligence Mapping"description="Configure call classification for each call tracking provider"actions={
        <button
        ...
        {mappings.map((mapping) => {
          ...
          return (
            ...
            >
              {/* Provider header */}
              <div
                className="flex items-center justify-between px-4 py-3"style={{ background: "var(--rtm-surface-raised, #F8FAFC)", borderBottom: "1px solid var(--rtm-border)"}}
              >
                <div>
                ...
                <button
                  className="text-xs font-medium px-2.5 py-1 rounded-lg border"style={{
                    color: "var(--rtm-blue)",
              ...
              <div className="divide-y divide-slate-100">
                {mapping.classifications.map((cls) => (
                  <div
                    key={cls.id}
                    className="flex items-start justify-between gap-4 px-4 py-3"style={{ background: "var(--rtm-surface)"}}
                  >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-1 mt-1.5">
                          {cls.keywords.map((kw) => (
                            <span
                              key={kw}
                              className="text-[11px] px-1.5 py-0.5 rounded font-mono"style={{
              ...
              {/* Add custom */}
              <div
                className="px-4 py-3"style={{ borderTop: "1px solid var(--rtm-border-light)", background: "var(--rtm-surface-raised, #F8FAFC)"}}
              >
                <button
                  className="text-xs font-medium flex items-center gap-1.5"style={{ color: "var(--rtm-blue)"}}
                >
