      ...
      <SectionWrapper title="Department Profile"description="Settings and Departments configuration">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="flex flex-col gap-0.5">
            ...
            <span
              className="text-xs px-2 py-0.5 rounded-full font-semibold w-fit"style={{ background: typeCfg.bg, color: typeCfg.color }}
            >
              {typeCfg.label}
            </span>
          </div>
        ...
        <div className="mt-4 pt-4 border-t"style={{ borderColor: "var(--rtm-border-light)"}}>
          <a
            href="/settings/departments"className="text-sm font-medium hover:underline"style={{ color: dept.accentColor }}
          >
      ...
      <SectionWrapper title="Enabled Modules"description="Toggle modules in Settings → Departments">
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {dept.modules.map((mod) => (
            ...
            >
              <div
                className="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0"style={{
          ...
          {dept.roles.map((role) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className="text-xs px-2 py-0.5 rounded-full font-semibold whitespace-nowrap capitalize"style={{ background: rstyle.bg, color: rstyle.color }}
                >
                  {role.type.replace("-", "")}
                </span>
              </div>
      ...
      <SectionWrapper title="Assigned KPIs"description="KPIs from Settings → Departments">
        <div className="space-y-2">
          {dept.kpis.map((kpi) => (
            <div
              key={kpi.id}
              className="flex items-center justify-between gap-3 py-2.5 px-3 rounded-lg border"style={{ background: "var(--rtm-bg)", borderColor: "var(--rtm-border-light)"}}
            >
              <div className="flex-1 min-w-0">
              ...
              {kpi.dashboardVisible && (
                <span
                  className="text-xs px-1.5 py-0.5 rounded font-semibold whitespace-nowrap"style={{ background: "var(--rtm-blue-light)", color: "var(--rtm-blue)"}}
                >
                  Dashboard
                </span>
              )}
        ...
        <div className="flex flex-wrap gap-2">
          {dept.integrations.map((int) => (
            <span
              ...
              className="text-xs px-3 py-1 rounded-full border font-medium"style={{ borderColor: "var(--rtm-border)", color: "var(--rtm-text-secondary)", background: "var(--rtm-bg)"}}
            >
              {int.displayName}
            </span>
          ))}
