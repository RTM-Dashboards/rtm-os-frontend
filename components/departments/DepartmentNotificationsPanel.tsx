...
export default function DepartmentNotificationsPanel({ accentColor, disabled }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Notification list */}
      <SectionWrapper
        title="Notification Center"description="Department, workflow, and escalation alerts — from Notification Center"actions={
        ...
        <div className="space-y-3">
          {MOCK_NOTIFICATIONS.map((notif) => {
            ...
            return (
              <div
                key={notif.id}
                className="flex items-start gap-3 p-4 rounded-lg border transition-colors hover:opacity-90"style={{
              ...
              >
                {/* Type badge */}
                <span
                  className="text-xs px-2 py-0.5 rounded-full font-semibold whitespace-nowrap flex-shrink-0 mt-0.5"style={{ background: ts.bg, color: ts.color }}
                >
                  {ts.label}
                </span>
