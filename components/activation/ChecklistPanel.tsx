  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-4 mb-4">
        <div>
          <p
            className="text-[11px] font-bold uppercase tracking-widest mb-0.5"style={{ color: "var(--rtm-blue)"}}
          >
        ...
        <button
          onClick={onClose}
          className="p-1.5 rounded-lg hover:bg-gray-100 transition-colors flex-shrink-0"style={{ color: "var(--rtm-text-muted)"}}
        >
      ...
      <div className="mb-5">
        ...
        <div
          className="h-2 rounded-full overflow-hidden"style={{ background: "var(--rtm-border)"}}
        >
          <div
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
        {record.checklist.map((item) => (
          <div
            key={item.key}
            className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg border"style={{
              background: item.completed ? "#F0FDF4": "var(--rtm-bg)",
              borderColor: item.completed ? "#BBF7D0": "var(--rtm-border)",
            }}
          >
            <div
              className="w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center"style={{
            ...
            <span
              className="text-xs font-medium"style={{ color: item.completed ? "#065F46": "var(--rtm-text-secondary)"}}
            >
              {item.label}
            </span>
          </div>
