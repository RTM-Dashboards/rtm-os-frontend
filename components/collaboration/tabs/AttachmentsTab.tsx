    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-[11px]" style={{ color: "var(--rtm-text-muted)" }}>
          File list shown is illustrative. Upload/download not yet implemented.
        </span>
      </div>
      ...
      {/* Upload zone — disabled: no file storage yet */}
      <div
        className="flex flex-col items-center gap-2 py-8 rounded-xl border-2 border-dashed opacity-40 cursor-not-allowed"
        style={{ borderColor: "var(--rtm-border)", color: "var(--rtm-text-muted)" }}
        title="Not yet available"
      >
        📎
        <p className="text-sm font-medium">Drop files here or click to upload</p>
        <p className="text-xs">Not yet available — file upload is deferred</p>
      </div>
    </div>
  );
}
...
function AttachmentRow({ att }: { att: TaskAttachment }) {
  ...
  return (
    ...
    >
      ...
      {/* Download — disabled: no file storage yet */}
      <div className="flex items-center gap-2 flex-shrink-0">
        <button
          disabled
          title="Not yet available"
          className="text-xs px-3 py-1.5 rounded-lg font-medium opacity-40 cursor-not-allowed"
