...
export default function WatchersTab({ watchers }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Explain */}
      <div
        className="p-4 rounded-xl text-sm"style={{ background: "var(--rtm-bg)", border: "1px solid var(--rtm-border)", color: "var(--rtm-text-secondary)"}}
      >
        ...
        <button
          className="mt-3 text-xs px-4 py-1.5 rounded-lg font-semibold text-white"style={{ background: "var(--rtm-blue)"}}
        >
          + Watch This Task
        </button>
      </div>
...
function WatcherRow({ watcher }: { watcher: TaskWatcher }) {
  return (
    <div
      className="flex items-center justify-between p-3 rounded-xl"style={{ background: "var(--rtm-surface)", border: "1px solid var(--rtm-border)"}}
    >
