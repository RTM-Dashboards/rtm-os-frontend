...
export default function ApprovalsTab({ approvals }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Request New */}
      <div
        className="p-4 rounded-xl"style={{ background: "var(--rtm-bg)", border: "1px solid var(--rtm-border)"}}
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            ...
            <input
              type="text"placeholder="e.g. Content Approval"className="w-full rounded-lg px-3 py-2 text-sm outline-none"style={{ background: "var(--rtm-surface)", border: "1px solid var(--rtm-border)", color: "var(--rtm-text-primary)"}}
            />
          </div>
          <div>
            <label className="text-[11px] font-semibold block mb-1"style={{ color: "var(--rtm-text-muted)"}}>Approver</label>
            <input
              type="text"placeholder="Select approver"className="w-full rounded-lg px-3 py-2 text-sm outline-none"style={{ background: "var(--rtm-surface)", border: "1px solid var(--rtm-border)", color: "var(--rtm-text-primary)"}}
            />
          </div>
        </div>
        <textarea
          rows={2}
          placeholder="Notes for the approver (optional)"className="w-full mt-3 resize-none rounded-lg px-3 py-2 text-sm outline-none"style={{ background: "var(--rtm-surface)", border: "1px solid var(--rtm-border)", color: "var(--rtm-text-primary)"}}
        />
...
function ApprovalCard({ approval }: { approval: ApprovalStep }) {
  ...
  return (
    ...
    >
      ...
      {approval.decision && (
        <div
          className="p-3 rounded-lg mb-3"style={{ background: "var(--rtm-bg)", border: "1px solid var(--rtm-border-light)"}}
        >
      ...
      {!isComplete && (
        <div className="flex items-center gap-2 mt-3 pt-3"style={{ borderTop: "1px solid var(--rtm-border-light)"}}>
          <button
            className="text-xs px-3 py-1.5 rounded-lg font-semibold text-white"style={{ background: "#059669"}}
          >
            Approve
          </button>
          <button
          ...
          <button
            className="text-xs px-3 py-1.5 rounded-lg font-semibold"style={{ background: "#FEF3C7", color: "#92400E"}}
          >
            ↩ Request Revision
          </button>
        </div>
