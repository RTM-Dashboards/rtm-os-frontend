...
export default function TopNav({ onMenuClick }: TopNavProps) {
  ...
  return (
    <header
      className="h-[60px] flex items-center justify-between px-4 sm:px-6 flex-shrink-0 sticky top-0 z-10"style={{
        background: "#FFFFFF",
    ...
    >
      ...
      <div className="flex items-center gap-3">
        <button
          onClick={onMenuClick}
          className="lg:hidden p-2 rounded-lg transition-colors"style={{ color: "var(--rtm-text-secondary)"}}
        ...
        <div className="hidden sm:flex items-center gap-3">
          {/* RTM blue accent bar */}
          <span
            className="w-0.5 h-5 rounded-full hidden lg:block"style={{ background: "linear-gradient(180deg, var(--rtm-blue) 0%, var(--rtm-blue-mid) 100%)"}}
          />
          <div className="flex items-baseline gap-2">
            ...
            {page.sub && (
              <span
                className="text-xs hidden md:inline"style={{ color: "var(--rtm-text-muted)"}}
              >
                {page.sub}
              </span>
            )}
      ...
      <div className="flex items-center gap-1.5 sm:gap-2">
        ...
        {/* Search — desktop */}
        <button
          className="hidden md:flex items-center gap-2 rounded-lg px-3 py-1.5 text-sm w-52 border transition-all"style={{
        ...
        >
          ...
          <kbd
            className="ml-auto text-[10px] rounded-md px-1.5 py-0.5 font-mono border"style={{
              background: "#ffffff",
              borderColor: "var(--rtm-border)",
              color: "var(--rtm-text-muted)",
            }}
        ...
        <div
          className="hidden lg:flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border"style={{
            background: "var(--rtm-blue-xlight)",
            borderColor: "var(--rtm-blue-light)",
          }}
        >
          <div
            className="w-1.5 h-1.5 rounded-full rtm-live-dot"style={{ background: "var(--rtm-blue)"}}
          />
        ...
        <div className="relative"ref={avatarRef}>
          ...
          {dropdownOpen && (
            <div
              className="absolute right-0 mt-2 w-56 rounded-xl shadow-lg py-1 z-50"style={{
                background: "#ffffff",
              ...
              <div className="py-1">
                {avatarLinks.map((item) => (
                  <button
                    ...
                    className="w-full text-left px-4 py-2 text-sm transition-colors"style={{ color: "var(--rtm-text-primary)"}}
                    onMouseEnter={(e) => (e.currentTarget.style.background = "var(--rtm-blue-xlight)")}
                    onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}
                  >
                    {item.label}
