...
export default function TaskAccessCard({
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <Link
            href={taskUrl}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold text-white transition-opacity hover:opacity-90"style={{ background: "var(--rtm-blue)"}}
          >
            Open Tasks
          </Link>
          <Link
            href={taskUrl}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold border transition-colors hover:opacity-80"style={{ borderColor: "#BFDBFE", color: "var(--rtm-blue)", background: "#fff"}}
          >
            {createLabel}
          </Link>
        </div>
  ...
  return (
    ...
    >
      {/* Header */}
      <div
        className="px-5 py-4 flex items-center justify-between gap-3"style={{ background: "var(--rtm-blue-xlight)", borderBottom: "1px solid #BFDBFE"}}
      >
        <div className="flex items-center gap-2">
      ...
      <div className="px-5 py-3 grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          <div
            key={label}
            className="rounded-lg p-3 text-center"style={{ background: bg }}
          >
        ...
        <div className="px-5 pb-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {examples.map((ex) => (
              <span
                key={ex}
                className="text-[11px] px-2 py-1 rounded-lg font-medium"style={{ background: "var(--rtm-bg)", color: "var(--rtm-text-secondary)", border: "1px solid var(--rtm-border)"}}
              >
                {ex}
              </span>
            ))}
      ...
      {/* Actions */}
      <div
        className="px-5 py-3 flex flex-wrap gap-2"style={{ borderTop: "1px solid var(--rtm-border-light)"}}
      >
        <Link
          href={taskUrl}
          className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-bold text-white transition-opacity hover:opacity-90"style={{ background: "var(--rtm-blue)"}}
        >
          Open Tasks
        </Link>
        <Link
          href={taskUrl}
          className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold border transition-colors hover:opacity-80"style={{ borderColor: "var(--rtm-border)", color: "var(--rtm-text-primary)"}}
        >
          {createLabel}
        </Link>
        <Link
          href={taskUrl}
          className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold border transition-colors hover:opacity-80"style={{ borderColor: "#BFDBFE", color: "var(--rtm-blue)", background: "var(--rtm-blue-light)"}}
        >
          Open Task Queue
        </Link>
      </div>
