...
export default function DeliverableCard({ deliverable, onTaskClick }: Props) {
  ...
  return (
    <div
      className="rtm-card overflow-hidden"style={{ borderLeft: hasBlockers ? "3px solid #EF4444": undefined }}
    >
      {/* Deliverable header */}
      <div
        className="px-5 py-4 cursor-pointer"style={{ borderBottom: expanded ? "1px solid var(--rtm-border-light)": "none"}}
        onClick={() => setExpanded((v) => !v)}
      >
        <div className="flex items-start gap-3">
          {/* Expand toggle */}
          <button
            className="mt-0.5 flex-shrink-0 w-5 h-5 rounded flex items-center justify-center transition-transform"style={{
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-2 mb-1.5">
              ...
              {hasBlockers && (
                <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full"style={{ background: "#FEF2F2", color: "#DC2626", border: "1px solid #FECACA"}}>
                  Blocked
                </span>
              )}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4">
              <span className="text-xs font-medium px-2 py-0.5 rounded-full"style={{ background: "var(--rtm-blue-xlight)", color: "var(--rtm-blue)"}}>
                {deliverable.department}
              </span>
              <div className="flex items-center gap-1.5">
                <span className="text-xs"style={{ color: "var(--rtm-text-muted)"}}>Owner:</span>
                <span
                  className="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold text-white"style={{ background: deliverable.owner.avatarColor }}
        ...
        <div className="divide-y"style={{ borderColor: "var(--rtm-border-light)"}}>
          {deliverable.tasks.map((task) => {
            ...
            return (
              <div
                key={task.id}
                className="px-5 py-3 flex flex-wrap items-center gap-3 cursor-pointer transition-colors"style={{ paddingLeft: "52px"}}
              ...
              >
                ...
                <span className="flex-1 min-w-0 text-sm font-medium leading-snug truncate"style={{ color: "var(--rtm-text-primary)"}}>
                ...
                <div className="flex items-center gap-3 flex-shrink-0">
                  ...
                  {task.dueDate && (
                    <span
                      className="text-xs whitespace-nowrap"style={{ color: overdue ? "#DC2626": "var(--rtm-text-muted)"}}
                    >
