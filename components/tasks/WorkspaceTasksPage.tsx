...
function KPICard({ label, value, color, bg }: { label: string; value: number; color: string; bg: string }) {
  return (
    <div
      className="rtm-card flex flex-col gap-1 px-5 py-4"style={{ borderLeft: `4px solid ${color}` }}
    >
...
function StatusBadge({ status }: { status: WorkspaceTaskStatus }) {
  ...
  return (
    <span
      className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap"style={{ background: `${color}18`, color }}
    >
      <span
...
function PriorityBadge({ priority }: { priority: WorkspaceTaskPriority }) {
  ...
  return (
    <span
      className="inline-block px-2 py-0.5 rounded text-xs font-bold"style={{ background: bg, color: text }}
    >
      {priority}
    </span>
  );
...
function BlockerFlag({ blocker }: { blocker: string | null }) {
  ...
  return (
    <span
      className="inline-flex items-center gap-1 text-xs font-semibold"style={{ color: "#DC2626"}}
      title={blocker}
    >
    ...
    <div className="rtm-card flex flex-wrap gap-3 px-4 py-3 items-center">
      ...
      <div className="relative flex-1 min-w-[180px]">
        ...
        <input
          ...
          placeholder="Search tasks, clients, services…"className="w-full pl-8 pr-3 py-1.5 text-sm rounded-lg border focus:outline-none"style={{
      ...
      <select
        ...
        className="text-sm py-1.5 px-3 rounded-lg border focus:outline-none"style={{
          background: "var(--rtm-bg)",
          border: "1px solid var(--rtm-border)",
          color: "var(--rtm-text-primary)",
        }}
      ...
      {(filters.search || filters.status !== "All"|| filters.priority !== "All") && (
        <button
          ...
          className="text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors"style={{ background: "var(--rtm-blue-light)", color: "var(--rtm-blue)"}}
        >
          Clear
        </button>
      )}
    ...
    <div className="flex flex-col gap-6 p-6 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-start gap-3">
          <div
            className="w-10 h-10 rounded-xl flex items-center justify-center text-xl flex-shrink-0 mt-0.5"style={{ background: `${accentColor}18` }}
          >
            {icon}
          </div>
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              <span
                className="text-xs font-bold uppercase tracking-widest px-2 py-0.5 rounded-full"style={{ background: `${accentColor}18`, color: accentColor }}
              >
                {department}
              </span>
            </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="rtm-card overflow-hidden">
              ...
              <thead>
                <tr style={{ borderBottom: "2px solid var(--rtm-border)", background: "var(--rtm-bg)"}}>
                  {[
                    ...
                    <th
                      key={h}
                      className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider"style={{ color: "var(--rtm-text-muted)", whiteSpace: "nowrap"}}
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {filtered.map((task, i) => {
                  ...
                  return (
                    <tr
                      key={task.id}
                      className="transition-colors"style={{ borderBottom: "1px solid var(--rtm-border-light)", background: rowBg }}
                    ...
                    >
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <div
                            className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold text-white flex-shrink-0"style={{ background: accentColor }}
                          >
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <span
                          className="text-xs font-medium"style={{ color: overduRow ? "#DC2626": "var(--rtm-text-muted)"}}
                        >
          ...
          {/* Table footer */}
          <div
            className="px-4 py-2 border-t flex items-center justify-between"style={{ borderColor: "var(--rtm-border)", background: "var(--rtm-bg)"}}
          >
