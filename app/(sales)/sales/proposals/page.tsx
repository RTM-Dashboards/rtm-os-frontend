...
type ProposalStatus =
  | "Draft"| "Internal Review"| "Pending Approval"| "Sent"| "Negotiation"| "Accepted"| "Rejected"| "Expired"| "Converted To Contract";
...
function CardHeader({ title, subtitle, actions }: { title: string; subtitle?: string; actions?: React.ReactNode }) {
  return (
    <div className="px-5 py-4 border-b flex items-center justify-between gap-3 flex-wrap"style={{ borderColor: "var(--rtm-border)"}}>
      <div>
  ...
  return (
    <div>
      ...
      <input type={type} value={value} placeholder={placeholder}
        ...
        className="w-full text-xs rounded-lg border px-3 py-2 focus:outline-none"style={{ background: "var(--rtm-bg)", borderColor: "var(--rtm-border)", color: "var(--rtm-text-primary)"}} />
    </div>
  );
}
    ...
    <Card>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs"style={{ borderCollapse: "collapse", minWidth: "1100px"}}>
          ...
          <tbody>
            {filtered.map((p, i) => (
              <tr key={p.id} className="hover:bg-blue-50 transition-colors cursor-pointer"style={{ borderBottom: "1px solid var(--rtm-border-light)", background: i % 2 === 0 ? "transparent": "var(--rtm-surface)"}}
                onClick={() => onSelect(p)}>
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-wrap gap-1">
                    ...
                    {p.services.length > 3 && (
                      <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full"style={{ background: "#F3F4F6", color: "#6B7280"}}>+{p.services.length - 3}</span>
                    )}
                  </div>
                </td>
    ...
    <Card>
      ...
      <div className="p-5 space-y-3">
        ...
        {selectedLineItems.map(li => (
          <div key={li.id} className="flex items-center justify-between px-4 py-3 rounded-lg border"style={{ background: "var(--rtm-bg)", borderColor: "var(--rtm-border)"}}>
        ...
        {showCatalog && (
          <div className="mt-4 space-y-2 pt-4 border-t"style={{ borderColor: "var(--rtm-border)"}}>
            ...
            {LINE_ITEM_CATALOG.map(li => (
              <div key={li.id} className="flex items-center justify-between px-4 py-3 rounded-lg border"style={{ background: selectedIds.has(li.id) ? "#F0FDF4": "var(--rtm-bg)", borderColor: selectedIds.has(li.id) ? "#A7F3D0": "var(--rtm-border)"}}>
                <div className="flex items-center gap-3">
                  <CategoryBadge cat={li.category} />
                  <div>
                    <p className="text-xs font-semibold"style={{ color: "var(--rtm-text-primary)"}}>{li.name}</p>
...
function LineItemBuilder({ lineItems, onUpdate, onRemove, onClone }: {
  ...
  return (
    <Card>
      <CardHeader title="Line Item Builder"subtitle={`${lineItems.length} line item${lineItems.length !== 1 ? "s": ""}`} />
      <div className="overflow-x-auto">
              ...
              <tr key={li.id} style={{ borderBottom: "1px solid var(--rtm-border-light)", background: i % 2 === 0 ? "transparent": "var(--rtm-surface)"}}>
                ...
                <td className="px-4 py-3">
                  <input type="number"min={1} max={10} value={li.quantity}
                    ...
                    className="w-14 text-xs rounded border px-2 py-1 text-center focus:outline-none"style={{ background: "var(--rtm-bg)", borderColor: "var(--rtm-border)", color: "var(--rtm-text-primary)"}} />
                </td>
                <td className="px-4 py-3">
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full"style={{ background: DELIVERY_STYLES[li.deliveryStandard].bg, color: DELIVERY_STYLES[li.deliveryStandard].text }}>
                    {li.deliveryStandard}
                  </span>
                </td>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input value={budget} onChange={e => setBudget(e.target.value)}
                className="w-full text-sm font-bold rounded-lg border pl-7 pr-3 py-2.5 focus:outline-none"style={{ background: "var(--rtm-bg)", borderColor: "#1D4ED8", color: "#1D4ED8"}} />
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {(["Budget Match","Growth Package","Full Service","Starter"] as AIModeOption[]).map(m => (
              <button key={m} onClick={() => setMode(m)}
                className="px-3 py-1.5 text-xs font-semibold rounded-lg border transition-all"style={{
  ...
  return (
    <div className="rounded-xl border p-5"style={{ background: "#F8FAFC", borderColor: "#E2E8F0"}}>
      <div className="flex items-start justify-between gap-4 mb-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full"style={{ background: confBg, color: confColor }}>
              {conf}% confidence
            </span>
          </div>
      ...
      <div className="mt-3">
        ...
        <div className="flex flex-wrap gap-1">
          {pkg.lineItems.map(l => (
            <span key={l} className="text-[10px] px-2 py-0.5 rounded-full border"style={{ background: "var(--rtm-surface)", color: "var(--rtm-text-secondary)", borderColor: "var(--rtm-border)"}}>
              {l}
            </span>
          ))}
        </div>
...
function DiscountPanel({ discount, onUpdate, setupTotal, recurringTotal }: {
  ...
  return (
    <Card>
      <CardHeader title="Discounts & Promo Codes"/>
      <div className="p-5 space-y-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <Select label="Discount Type"value={discount.type}
                    ...
                    <div className="flex flex-wrap gap-1">
                      {li.recurringTasks.length > 0
                        ? li.recurringTasks.slice(0, 3).map(t => (
                          <span key={t} className="text-[9px] px-1.5 py-0.5 rounded-full"style={{ background: "#F0F9FF", color: "#0369A1"}}>{t}</span>
                        ))
                        : <span style={{ color: "var(--rtm-text-muted)"}}>Setup only</span>
                      }
                      {li.recurringTasks.length > 3 && (
                        <span className="text-[9px] px-1.5 py-0.5 rounded-full"style={{ background: "#F3F4F6", color: "#6B7280"}}>+{li.recurringTasks.length - 3}</span>
                      )}
                    </div>
                  </td>
          ...
          <div className="flex flex-wrap gap-2">
            {TERMS.map(t => (
              <button key={t} onClick={() => onChange({ ...config, term: t })}
                className="px-4 py-2 text-xs font-semibold rounded-lg border transition-all"style={{
          ...
          {config.term === "Custom"&& (
            <div className="mt-2">
              ...
              <input type="number"min={1} max={120} value={config.customMonths}
                ...
                className="text-xs rounded-lg border px-3 py-2 focus:outline-none w-28"style={{ background: "var(--rtm-bg)", borderColor: "var(--rtm-border)", color: "var(--rtm-text-primary)"}} />
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="sm:col-span-2">
            ...
            <textarea value={config.cancellationTerms} rows={2}
              ...
              className="w-full text-xs rounded-lg border px-3 py-2 focus:outline-none resize-none"style={{ background: "var(--rtm-bg)", borderColor: "var(--rtm-border)", color: "var(--rtm-text-primary)"}} />
          </div>
        </div>
          ...
          {steps.map((s, i) => (
            <React.Fragment key={s.key}>
              <div className="flex flex-col items-center gap-2 min-w-[100px]">
                <div className="w-12 h-12 rounded-full flex items-center justify-center text-xl border-2"style={{
                    background: s.done ? "#F0FDF4": "var(--rtm-bg)",
                    borderColor: s.done ? "#059669": "var(--rtm-border)",
                  }}>
                  {s.icon}
                </div>
                <span className="text-[10px] font-semibold text-center max-w-[80px] leading-tight"style={{ color: s.done ? "#059669": "var(--rtm-text-muted)"}}>
                  {s.label}
                </span>
              </div>
              {i < steps.length - 1 && (
                <div className="flex-1 h-0.5 min-w-[24px] mb-5"style={{ background: s.done ? "#059669": "var(--rtm-border)"}} />
              )}
            </React.Fragment>
          ))}
        </div>
    ...
    <Card>
      ...
      <div className="p-5 space-y-4">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {s.riskFactors.map((r) => (
              <span key={r} className="text-xs px-3 py-1.5 rounded-lg border"style={{ background: "#FFF7ED", color: "#C2410C", borderColor: "#FED7AA"}}>
                 {r}
              </span>
            ))}
          </div>
      ...
      <div className="flex overflow-x-auto rounded-xl border"style={{ background: "var(--rtm-surface)", borderColor: "var(--rtm-border)"}}>
        {BUILDER_TABS.map(t => (
          <button key={t.key} onClick={() => setBuilderTab(t.key as typeof builderTab)}
            className="flex items-center gap-1.5 px-4 py-3 text-xs font-semibold whitespace-nowrap transition-colors"style={{
