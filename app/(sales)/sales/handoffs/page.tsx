...
function ChecklistItem({ label, done }: { label: string; done: boolean }) {
  return (
    <div className="flex items-center gap-2.5 py-1.5">
      <span
        className="flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-[11px] font-bold border"
        style={{
      ...
      <span
        ...
        style={{ color: done ? "var(--rtm-text-primary)" : "var(--rtm-text-muted)" }}
      >
        {label}
      </span>
    </div>
...
function HandoffDetailDrawer({
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "proposal" | "billing" | "services" | "affiliate" | "activation" | "workflow" | "tasks" | "notes"
  >("overview");
  ...
  return (
    ...
    >
      <div
        className="ml-auto flex flex-col w-full max-w-2xl h-full overflow-hidden"
        style={{
      ...
      >
        ...
        >
          ...
          <button
            onClick={onClose}
            className="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center hover:opacity-80 transition-opacity text-lg"
            ...
            <div key={label} className="px-4 py-3 text-center">
              ...
              {label === "Priority" ? (
                <span
                  ...
                  style={priorityColor(handoff.priority)}
                >
                  {value}
                </span>
              ) : (
                ...
                <div className="flex flex-wrap gap-1.5">
                  {handoff.servicesSold.map(s => (
                    <span
                      ...
                      style={{ background: serviceColor(s) + "18", color: serviceColor(s), border: `1px solid ${serviceColor(s)}40` }}
                    >
                      {s}
                    </span>
                  ))}
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-lg p-3 border" style={{
                ...
                <p className="text-[10px] font-bold uppercase tracking-wide mb-1" style={{
                  color: handoff.billingRequirements.finalApprovalStatus === "Approved" ? "#059669" : "#DC2626"
                }}>Final Approval Status</p>
                <p className="text-xs font-semibold" style={{
                  color: handoff.billingRequirements.finalApprovalStatus === "Approved" ? "#065F46" : "#991B1B"
                }}>{handoff.billingRequirements.finalApprovalStatus}</p>
              </div>
                ...
                <div key={svc.service} className="rounded-xl border p-4" style={{ background: "var(--rtm-bg)", borderColor: "var(--rtm-border)" }}>
                  <div className="flex items-center justify-between mb-3">
                    <span
                      ...
                      style={{ background: serviceColor(svc.service) + "18", color: serviceColor(svc.service), border: `1px solid ${serviceColor(svc.service)}40` }}
                    >
                      {svc.service}
                    </span>
      ...
      render: (_v, row) => (
        <div className="flex gap-1">
          <button
            ...
            className="text-[10px] font-bold px-2 py-1 rounded transition-opacity hover:opacity-80"
            style={{ background: "var(--rtm-blue-light)", color: "var(--rtm-blue)" }}
          >
            View
          </button>
    ...
    <div className="overflow-x-auto pb-2">
      <div className="flex gap-3 min-w-max">
        {stageCounts.map(({ stage, count, revenue }) => (
          <div
            key={stage}
            className="w-48 rounded-xl border overflow-hidden flex-shrink-0"
              ...
              {data
                ...
                .map(h => (
                  <button
                    key={h.id}
                    onClick={() => onSelect(h)}
                    className="w-full text-left rounded-lg px-2.5 py-2 border transition-shadow hover:shadow-sm"
  ...
  return (
    <div className="min-h-screen" style={{ background: "var(--rtm-bg)" }}>
      <div className="max-w-screen-2xl mx-auto px-6 py-6 space-y-6">
        ...
        <PageHeader
          breadcrumb="Sales"
          title="Sales Handoff Center"
          description="Manage approved proposals, billing requests, invoice readiness, activation readiness, and revenue handoff from Sales to Billing."
          actions={
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <button
                  ...
                  style={{ background: bg, color: bg === "var(--rtm-surface)" ? color : "#fff", borderColor: bg === "var(--rtm-surface)" ? "var(--rtm-border)" : bg }}
                >
                  {label}
                </button>
              ))}
              <Link
                ...
                className="text-xs font-semibold px-3 py-1.5 rounded-lg border transition-opacity hover:opacity-80 inline-flex items-center gap-1"
                style={{ background: "var(--rtm-surface)", color: "var(--rtm-blue)", borderColor: "var(--rtm-border)" }}
              >
        ...
        <SectionWrapper title="Handoff Pipeline &amp; Queue">
          <div className="flex flex-wrap items-center gap-3 mb-5">
            ...
            <input
              type="text"
              placeholder="Search client, rep, proposal…"
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              className="text-xs px-3 py-2 rounded-lg border flex-1 min-w-[200px] max-w-xs"
              style={{ background: "var(--rtm-surface)", borderColor: "var(--rtm-border)", color: "var(--rtm-text-primary)" }}
            />
          ...
          <div className="flex flex-wrap gap-3">
            {[
              ...
              <Link
                key={label}
                href={href}
                className="flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-lg border transition-opacity hover:opacity-80"
