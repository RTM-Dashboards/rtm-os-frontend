...
const teamMembers = [
  { name: "Ryan B.", role: "Department Head", access: "Full Access", status: "Active", clients: 15, lastActive: "Today, 8:55 AM"},
  { name: "Tina G.", role: "Sr. Sales Executive", access: "Edit", status: "Active", clients: 10, lastActive: "Today, 9:20 AM"},
  { name: "Carlos V.", role: "Sales Executive", access: "Edit", status: "Active", clients: 7, lastActive: "Yesterday, 4:45 PM"},
  { name: "Megan S.", role: "Sales Coordinator", access: "View", status: "Active", clients: 5, lastActive: "Today, 7:30 AM"},
  { name: "David L.", role: "Business Dev Rep", access: "View", status: "Inactive", clients: 0, lastActive: "5 days ago"},
];

const workspacePermissions = [
  { label: "Manage Leads", enabled: true },
  { label: "Convert Leads to Clients", enabled: true },
  { label: "Create Proposals", enabled: true },
  { label: "Edit Pricing", enabled: false },
  { label: "View Pipeline Analytics", enabled: true },
  { label: "Export CRM Data", enabled: false },
];

const notificationPrefs = [
  { label: "New lead submitted", channel: "Email + Slack", enabled: true },
  { label: "Lead follow-up due", channel: "Slack", enabled: true },
  { label: "Proposal viewed by client", channel: "Email", enabled: true },
  { label: "Deal closed (won)", channel: "Email + Slack", enabled: true },
  { label: "Deal closed (lost)", channel: "Email", enabled: false },
];

const defaultTaskSettings = [
  { label: "Default Priority", value: "High"},
  { label: "Default Assignee", value: "Sales Executive"},
  { label: "Follow-up Offset", value: "1 business day"},
  { label: "Proposal Expiry", value: "14 days"},
  { label: "Task Template", value: "Sales Standard"},
];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border p-5"style={{ background: "var(--rtm-surface)", borderColor: "var(--rtm-border)"}}>
        <h3 className="text-sm font-bold mb-4"style={{ color: "var(--rtm-text-primary)"}}>
          Department Head / Workspace Admin
        </h3>
        <div className="flex items-center gap-4 p-4 rounded-lg"style={{ background: "var(--rtm-bg)", border: "1px solid var(--rtm-border-light)"}}>
          <div
            className="w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-lg flex-shrink-0"style={{ background: "var(--rtm-blue)"}}
          >
            R
          </div>
          <div className="flex-1">
            <p className="font-bold text-sm"style={{ color: "var(--rtm-text-primary)"}}>Ryan B.</p>
            <p className="text-xs mt-0.5"style={{ color: "var(--rtm-text-secondary)"}}>ryan@rtm.io · Sales Department Head</p>
          </div>
          <span className="text-[11px] font-bold px-2.5 py-1 rounded-full"style={{ background: "#1B4FD818", color: "#1B4FD8"}}>
            Admin
          </span>
        </div>
      </div>
            ...
            <thead>
              <tr style={{ borderBottom: "1px solid var(--rtm-border-light)"}}>
                {["Member", "Role", "Access Level", "Status", "Assigned Clients", "Last Active"].map((h) => (
                  <th key={h} className="px-4 py-3 text-left text-xs font-semibold whitespace-nowrap"style={{ color: "var(--rtm-text-muted)"}}>
                    {h}
                  </th>
                ))}
              </tr>
                ...
                <tr key={m.name} style={{ borderBottom: i < teamMembers.length - 1 ? "1px solid var(--rtm-border-light)": "none"}}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      <div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0"style={{ background: "var(--rtm-blue)"}}>
                        {m.name.charAt(0)}
                      </div>
                  ...
                  <td className="px-4 py-3">
                    <span className="text-[11px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap"style={{ background: `${accessColors[m.access]}18`, color: accessColors[m.access] }}>
                      {m.access}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-[11px] font-bold px-2 py-0.5 rounded-full"style={m.status === "Active"? { background: "#05966918", color: "#059669"} : { background: "#78716C18", color: "#78716C"}}>
                      {m.status}
                    </span>
                  </td>
        ...
        <div className="space-y-2">
          {permissions.map((perm, i) => (
            <div key={perm.label} className="flex items-center justify-between p-3 rounded-lg"style={{ background: "var(--rtm-bg)", border: "1px solid var(--rtm-border-light)"}}>
              ...
              <button
                onClick={() => setPermissions((prev) => prev.map((p, idx) => (idx === i ? { ...p, enabled: !p.enabled } : p)))}
                className="relative inline-flex h-5 w-9 items-center rounded-full transition-colors flex-shrink-0"style={{ background: perm.enabled ? "#1B4FD8": "var(--rtm-border)"}}
              >
          ...
          {notifications.map((notif, i) => (
            <div key={notif.label} className="flex items-center justify-between p-3 rounded-lg"style={{ background: "var(--rtm-bg)", border: "1px solid var(--rtm-border-light)"}}>
              ...
              <button
                ...
                className="relative inline-flex h-5 w-9 items-center rounded-full transition-colors flex-shrink-0"style={{ background: notif.enabled ? "#1B4FD8": "var(--rtm-border)"}}
              >
