...
type OffboardingStatus =
  | "Offboarding Triggered"| "In Progress"| "Waiting on Billing"| "Waiting on AM"| "Department Shutdown"| "Access Removal"| "Final Report"| "Client Closure"| "Completed";

type CancellationStatus =
  | "Approved"| "Billing Cleared"| "Pending Balance"| "Billing Hold"| "Escalated"| "Closed";

type BillingStatus =
  | "Cleared"| "Pending Review"| "Outstanding Balance"| "Final Invoice Sent"| "Written Off"| "Closed";
      ...
      {steps.map((step, i) => (
        <React.Fragment key={step.label}>
          <div className="flex flex-col items-center gap-1.5 min-w-[80px]">
            <div
              className="w-10 h-10 rounded-full flex items-center justify-center text-lg border-2"style={{ borderColor: step.color, background: `${step.color}18` }}
            >
              {step.icon}
            </div>
            <span
              className="text-[10px] font-semibold text-center leading-tight"style={{ color: "var(--rtm-text-secondary)", whiteSpace: "pre-line"}}
            >
              {step.label}
            </span>
          </div>
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-3">
        <div className="flex-1 h-2 rounded-full"style={{ background: "var(--rtm-border)"}}>
          <div
            className="h-2 rounded-full transition-all"style={{ width: `${pct}%`, background: pct === 100 ? "#059669": "var(--rtm-blue)"}}
          />
        </div>
...
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"style={{ background: "rgba(15,28,56,0.45)"}}>
      <div
        className="w-full max-w-lg rounded-2xl border shadow-2xl flex flex-col max-h-[90vh]"style={{ background: "var(--rtm-surface)", borderColor: "var(--rtm-border)"}}
      >
        <div
          className="flex items-center justify-between px-6 py-4 flex-shrink-0"style={{ borderBottom: "1px solid var(--rtm-border-light)"}}
        >
          <h3 className="text-base font-bold"style={{ color: "var(--rtm-text-primary)"}}>{title}</h3>
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-full flex items-center justify-center text-lg hover:opacity-70 transition-opacity"style={{ background: "var(--rtm-border-light)", color: "var(--rtm-text-secondary)"}}
          >
            ×
          </button>
        </div>
...
function Input({ defaultValue, placeholder }: { defaultValue?: string; placeholder?: string }) {
  return (
    <input
      className="w-full px-3 py-2 rounded-lg border text-sm"style={{ borderColor: "var(--rtm-border)", background: "var(--rtm-surface)", color: "var(--rtm-text-primary)"}}
...
function Textarea({ placeholder, rows = 3 }: { placeholder?: string; rows?: number }) {
  return (
    <textarea
      className="w-full px-3 py-2 rounded-lg border text-sm resize-none"style={{ borderColor: "var(--rtm-border)", background: "var(--rtm-surface)", color: "var(--rtm-text-primary)"}}
      placeholder={placeholder}
      rows={rows}
    />
  );
  ...
  return (
    <Modal title="Request Billing Clearance"onClose={onClose}>
      <div className="flex flex-col gap-4">
        ...
        <div
          className="flex items-start gap-3 p-3 rounded-lg border text-xs"style={{ background: "#FFFBEB", borderColor: "#FDE68A", color: "#B45309"}}
        >
    ...
    <Modal title="Upload Final Report"onClose={onClose}>
      <div className="flex flex-col gap-4">
        ...
        <FieldRow label="File Upload">
          <div
            className="border-2 border-dashed rounded-lg p-6 text-center"style={{ borderColor: "var(--rtm-border)", color: "var(--rtm-text-muted)"}}
          >
  ...
  return (
    <Modal title="Complete Offboarding"onClose={onClose}>
      <div className="flex flex-col gap-4">
        ...
        <div
          className="flex items-start gap-3 p-3 rounded-lg border text-xs"style={{ background: "#EFF6FF", borderColor: "#BFDBFE", color: "#1D4ED8"}}
        >
...
export default function OffboardingPage() {
  ...
  return (
    <div
      className="min-h-screen"style={{ background: "var(--rtm-bg)", color: "var(--rtm-text-primary)", fontFamily: "var(--rtm-font-sans, system-ui, sans-serif)"}}
    >
      {/*  Header  */}
      ...
      <div className="px-6 py-3">
        <TaskAccessCard
          context="Offboarding"variant="banner"counters={{ open: 7, overdue: 2, dueToday: 3, completed: 18 }}
          createLabel="Create Offboarding Task"examples={["Create Shutdown Task", "Access Removal Task", "Final Report", "Billing Close"]}
        />
      </div>
      ...
      {/*  KPI Cards  */}
        ...
        <div className="flex items-center gap-0 border-b"style={{ borderColor: "var(--rtm-border)"}}>
          {TABS.map((tab) => (
            <button
              ...
              className="px-4 py-2.5 text-sm font-semibold border-b-2 transition-colors"style={{
        ...
        {activeTab === "queue"&& (
          <SectionWrapper
            title="Main Offboarding Queue"description="All active and recent offboarding cases"actions={
              ...
              <table className="w-full text-xs">
                ...
                <tbody>
                  {mockClients.map((c, idx) => (
                    <tr
                      key={c.id}
                      className="cursor-pointer hover:bg-blue-50/40 transition-colors"style={{ borderBottom: "1px solid var(--rtm-border-light)", background: idx % 2 === 0 ? "var(--rtm-surface)": "var(--rtm-surface-alt, #F8FAFC)"}}
        ...
        {activeTab === "checklist"&& (
          <div className="flex flex-col gap-4">
            {/* Client selector */}
            <SectionWrapper
              title="Select Client"description="Choose a client to view and manage their offboarding checklist"actions={
                <div className="flex gap-2 flex-wrap">
                  {mockClients.map((c) => (
                    <button
                      key={c.id}
                      onClick={() => setSelectedClient(c)}
                      className="px-3 py-1.5 rounded-lg border text-xs font-semibold transition-all"style={{
            ...
            {selectedClient && (
              <SectionWrapper
                ...
                description="Full checklist by category: Billing, AM, Departments, Access, and Final Deliverables"actions={
                  <div className="flex gap-2">
        ...
        {activeTab === "departments"&& (
          <SectionWrapper
            title="Department Shutdown Table"description="Track shutdown status per department for each offboarding client"actions={<Btn variant="primary"small>+ Create Task</Btn>}
            noPadding
          >
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
        ...
        {activeTab === "access"&& (
          <SectionWrapper
            title="Access Removal Table"description="Track platform access removal status for all offboarding clients"actions={<Btn variant="primary"small onClick={() => selectedClient && setModal({ kind: "removeAccess", record: selectedClient })}>+ Mark Access Removed</Btn>}
            noPadding
          >
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
        ...
        {activeTab === "events"&& (
          <SectionWrapper
            title="Recent Offboarding Events"description="Full event log across all offboarding cases"noPadding
          >
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  ...
                  {mockClients.flatMap((c) =>
                    c.recentEvents.map((e, idx) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <span
                            className="px-2 py-0.5 rounded text-[11px] font-semibold"style={{
                              background:
          ...
          <div className="flex flex-wrap gap-3">
            {[
              ...
              <Link key={link.href} href={link.href}>
                <div
                  className="flex items-center gap-2 px-3 py-2 rounded-lg border text-xs font-medium hover:shadow-sm transition-shadow cursor-pointer"style={{ borderColor: "var(--rtm-border)", color: "var(--rtm-text-secondary)", background: "var(--rtm-surface)"}}
                >
