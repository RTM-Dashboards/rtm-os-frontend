    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <button
            className="text-sm font-semibold px-4 py-2 rounded-lg border transition-colors"style={{
          ...
          <button
            className="text-sm font-semibold px-4 py-2 rounded-lg transition-colors text-white"style={{ background: "var(--rtm-blue)"}}
          >
            New Activation
          </button>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-4">
        <KpiCard
          title="Total Activations"value={String(ACTIVATION_RECORDS.length)}
          subtitle="All records"iconBg="var(--rtm-blue-xlight)"iconColor="var(--rtm-blue)"icon={<IconRocket className="w-5 h-5"/>}
        />
        <KpiCard
          title="Ready For Launch"value={String(readyForLaunch)}
          subtitle="Checklist complete"iconBg="#F0FDF4"iconColor="#059669"icon={
        ...
        />
        <KpiCard
          title="Currently Launching"value={String(launching)}
          subtitle="In active launch"iconBg="#EEF2FF"iconColor="#4F46E5"icon={
            <svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24">
        ...
        />
        <KpiCard
          title="Pending Payment"value={String(pendingPayment)}
          subtitle="Awaiting confirmation"iconBg="#FEF2F2"iconColor="#DC2626"icon={
            <svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24">
        ...
        />
        <KpiCard
          title="Pending Contract"value={String(pendingContract)}
          subtitle="Contract not signed"iconBg="#FFFBEB"iconColor="#D97706"icon={
            <svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24">
        ...
        />
        <KpiCard
          title="Revenue Waiting"value={formatCurrency(revenueWaiting)}
          subtitle="Not yet launched"iconBg="#F5F3FF"iconColor="#7C3AED"icon={
            <svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24">
      ...
      {blocked > 0 && (
        <div
          className="rounded-xl px-4 py-3 flex items-center gap-3"style={{ background: "#FEF2F2", border: "1px solid #FECACA"}}
        >
          <div
            className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0"style={{ background: "#EF4444"}}
          >
      ...
      {/* ── Tab Navigation ────────────────────────────────────────── */}
      <div
        className="rounded-xl border overflow-hidden"style={{ background: "var(--rtm-surface)", borderColor: "var(--rtm-border)", boxShadow: "0 1px 3px rgba(15,28,56,0.05)"}}
      >
        {/* Tab bar */}
        <div
          className="flex overflow-x-auto"style={{ borderBottom: "1px solid var(--rtm-border)"}}
        >
          {TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className="flex items-center gap-2 px-5 py-3.5 text-sm font-semibold whitespace-nowrap transition-all relative flex-shrink-0"style={{
