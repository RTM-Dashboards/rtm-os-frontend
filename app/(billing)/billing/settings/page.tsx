...
const teamMembers = [
  { name: "Priya N.", role: "Department Head", access: "Full Access", status: "Active", clients: 20, lastActive: "Today, 9:00 AM"},
  { name: "Tom W.", role: "Sr. Billing Specialist", access: "Edit", status: "Active", clients: 14, lastActive: "Today, 8:30 AM"},
  { name: "Dana F.", role: "Billing Coordinator", access: "Edit", status: "Active", clients: 9, lastActive: "Yesterday, 6:00 PM"},
  { name: "Kevin O.", role: "Accounts Receivable", access: "View", status: "Active", clients: 6, lastActive: "Today, 10:15 AM"},
  { name: "Janelle R.", role: "Billing Analyst", access: "View", status: "Inactive", clients: 0, lastActive: "1 week ago"},
];

const workspacePermissions = [
  { label: "View Invoices", enabled: true },
  { label: "Create & Send Invoices", enabled: true },
  { label: "Process Payments", enabled: true },
  { label: "Issue Refunds", enabled: false },
  { label: "Edit Billing Plans", enabled: false },
  { label: "Export Financial Reports", enabled: true },
];

const notificationPrefs = [
  { label: "Invoice created", channel: "Email", enabled: true },
  { label: "Payment received", channel: "Email + Slack", enabled: true },
  { label: "Invoice overdue (7 days)", channel: "Email + Slack", enabled: true },
  { label: "Subscription renewal upcoming", channel: "Email", enabled: true },
  { label: "Refund processed", channel: "Email", enabled: false },
];

const defaultTaskSettings = [
  { label: "Default Priority", value: "High"},
  { label: "Default Assignee", value: "Billing Specialist"},
  { label: "Invoice Due Offset", value: "Net 30"},
  { label: "Auto-reminder", value: "7 days before due"},
  { label: "Task Template", value: "Billing Standard"},
];
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border p-5"style={{ background: "var(--rtm-surface)", borderColor: "var(--rtm-border)"}}>
        ...
        <div className="flex items-center gap-4 p-4 rounded-lg"style={{ background: "var(--rtm-bg)", border: "1px solid var(--rtm-border-light)"}}>
          <div className="w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-lg flex-shrink-0"style={{ background: "var(--rtm-blue)"}}>P</div>
          <div className="flex-1">
            <p className="font-bold text-sm"style={{ color: "var(--rtm-text-primary)"}}>Priya N.</p>
            <p className="text-xs mt-0.5"style={{ color: "var(--rtm-text-secondary)"}}>priya@rtm.io · Billing Department Head</p>
          </div>
