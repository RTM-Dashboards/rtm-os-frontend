  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-5 gap-4">
        <KpiCard
          title="Ready For Activation"value={String(ready.length)}
          trend="up"trendValue="2"iconBg="#F0F9FF"iconColor="#0891B2"icon={<svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M5 13l4 4L19 7"/></svg>}
        />
        <KpiCard
          title="Pending Payment"value={String(pendingPayment.length)}
        ...
        />
        <KpiCard
          title="On Hold"value={String(onHold.length)}
          trend="neutral"trendValue="unchanged"iconBg="#FEF2F2"iconColor="#DC2626"icon={<svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M10 9v6m4-6v6m7-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>}
        />
        <KpiCard
          title="Not Ready"value={String(notReady.length)}
          trend="neutral"trendValue="unchanged"iconBg="#F9FAFB"iconColor="#6B7280"icon={<svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>}
        />
        <KpiCard
          title="Pipeline Revenue"value={`$${totalRevenue.toLocaleString()}`}
          trend="up"trendValue="$3,500"iconBg="#F5F3FF"iconColor="#7C3AED"icon={<svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/></svg>}
        />
      </div>

      {/* Activation Flow */}
      <div
        className="rounded-xl border p-5"style={{ background: "linear-gradient(135deg, #F5F3FF 0%, #EFF6FF 100%)", borderColor: "#DDD6FE"}}
      >
      ...
      <SectionWrapper
        title="Activation Queue"description="All clients in the activation pipeline"actions={
          ...
          <table className="min-w-full">
            ...
            <tbody>
              {filtered.map((a) => (
                <tr
                  key={a.id}
                  className="hover:bg-[#F9FAFB] transition-colors"style={{
                ...
                >
                  ...
                  <Td>
                    <div className="flex gap-1.5 flex-wrap">
                      <ActionBtn
                        label="Approve Activation"variant={a.activationStatus === "Ready For Activation"? "primary": "secondary"}
      ...
      <SectionWrapper title="Activation Cards"description="Per-client activation readiness detail">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {activationQueue.map((a) => (
            <div
              key={a.id}
              className="rounded-xl border p-5 space-y-4"style={{
                background: a.activationStatus === "Ready For Activation"? "#F0FFF4": a.activationStatus === "On Hold"? "#FFF7F7": "var(--rtm-bg)",
            ...
            >
              ...
              <textarea
                rows={2}
                placeholder="Add activation note…"value={notes[a.id] ?? ""}
