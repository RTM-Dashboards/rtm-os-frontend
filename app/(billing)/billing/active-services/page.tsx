 ...
 * active client service subscriptions derived from real Business records.
 *
 * DATA SOURCE: /api/clients and /api/businesses (Postgres via Prisma)
 * Fetched through lib/account-management/am-client-data (shared data layer).
 *
 * No mock data. No fallback. An empty table means zero real records exist.
 *
 * One BusinessClient per Business (domain). Services come from Business.activeServices[].
 * MRR is derived from Business.monthlyValueCents / 100, split across active services.
 * Status is derived from invoiceStatus / paymentStatus / billingStatus / activationStatus.
 *
 * EDIT PATH: Client Portfolio → Update Billing Fields (UpdateStatusModal)
 *   — the only place to edit a client's activeServices, via patchBusiness().
 */
...
import { computeHealth } from "@/lib/mock/master-clients";
...
/**
 * Derive a user-facing service status from business billing + activation fields.
 * Uses computeHealth (from billing/client-portfolio established pattern) to map
 * billing fields → health label, then maps that to ServiceStatus.
 */
function deriveServiceStatus(b: BusinessClient): ServiceStatus {
  // computeHealth expects { billingStatus, paymentStatus, cancellationStatus, activationStatus }
  // Map Business fields to the expected shape.
  const cancellationStatus =
    b.cancellationStatus === "Requested" ||
    b.cancellationStatus === "In Review" ||
    b.cancellationStatus === "Approved"
      ? (b.cancellationStatus as "Requested" | "In Review" | "Approved")
      : "None";

  const activationStatus =
    b.activationStatus === "active"
      ? "Active"
      : b.activationStatus === "suspended"
      ? "Suspended"
  ...
  const health = computeHealth({
    billingStatus:      b.billingStatus as Parameters<typeof computeHealth>[0]["billingStatus"],
    paymentStatus:      b.paymentStatus as Parameters<typeof computeHealth>[0]["paymentStatus"],
    cancellationStatus,
    activationStatus:   activationStatus as Parameters<typeof computeHealth>[0]["activationStatus"],
  });

  if (health === "Critical") return "Critical";
  if (health === "At Risk" || b.paymentStatus === "Overdue") return "At Risk";
  if (b.billingStatus === "Pending" && b.activationStatus === "inactive") return "New";
  if (b.billingStatus === "Pending") return "Pausing";
  return "Active";
}
...
/** Expand a BusinessClient into one ServiceRow per activeServices entry. */
function expandBusinessToRows(b: BusinessClient): ServiceRow[] {
  if (b.activeServices.length === 0) return [];
  const count = b.activeServices.length;
  const perService = Math.floor(b.monthlyValue / count);
  const remainder = b.monthlyValue - perService * count;
  const derivedStatus = deriveServiceStatus(b);

  return b.activeServices.map((svcName, idx) => ({
    rowKey:      `${b.id}__${idx}`,
    businessId:  b.id,
...
export default function BillingServicesPage() {
  ...
  // Expand all businesses into service rows (only those with at least one service)
  const allRows: ServiceRow[] = businesses.flatMap(expandBusinessToRows);
  ...
  // Unique business count
  const uniqueBusinessCount = new Set(allRows.map((r) => r.businessId)).size;
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: "var(--rtm-text-secondary)" }}>
          All current client service subscriptions — {allRows.length} subscription{allRows.length !== 1 ? "s" : ""} across {uniqueBusinessCount} business{uniqueBusinessCount !== 1 ? "es" : ""}.
          Sourced from real Business records in Postgres. Billing is the source of truth for active services.
        </p>
      </div>
      ...
      {!loading && !fetchError && businesses.length === 0 && (
        <div className="rounded-xl border p-12 text-center space-y-3" style={{ borderColor: "var(--rtm-border-light)", background: "var(--rtm-bg)" }}>
          ...
          <p className="text-base font-bold" style={{ color: "var(--rtm-text-primary)" }}>No active services yet</p>
          <p className="text-sm" style={{ color: "var(--rtm-text-muted)" }}>
            Active services appear here once client Business records exist with services recorded.
            Use <strong>Client Portfolio → Update Billing Fields</strong> to add services to a business.
          </p>
        </div>
      )}
      ...
      {/* KPI Summary — only shown when there is data */}
      {!loading && !fetchError && businesses.length > 0 && (
        <>
          ...
          >
            <div className="overflow-x-auto rounded-lg border" style={{ borderColor: "var(--rtm-border-light)" }}>
              <table className="min-w-full">
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-3 py-8 text-center text-sm" style={{ color: "var(--rtm-text-muted)" }}>
                    ...
                    <tr key={row.rowKey}
                      ...
                      <Td>
                        <div>
                          <span className="font-semibold" style={{ color: "var(--rtm-text-primary)" }}>{row.clientName}</span>
                          <span className="block text-[11px]" style={{ color: "var(--rtm-text-muted)" }}>{row.domain}</span>
                        </div>
                      </Td>
            ...
            <div className="mt-4 flex items-center justify-between px-1">
              <p className="text-xs" style={{ color: "var(--rtm-text-muted)" }}>
                Showing {filtered.length} subscription{filtered.length !== 1 ? "s" : ""} across {new Set(filtered.map((r) => r.businessId)).size} business{new Set(filtered.map((r) => r.businessId)).size !== 1 ? "es" : ""}
              </p>
