  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="h-3 rounded-full overflow-hidden"style={{ background: `${color}20` }}>
        <div
          className="h-3 rounded-full transition-all duration-500"style={{ width: `${pct}%`, background: `linear-gradient(90deg, ${color}CC, ${color})` }}
        />
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        />
        <KpiCard
          title="ARR"value={`$${(arr / 1000).toFixed(0)}k`}
        ...
        />
        <KpiCard
          title="Collected This Month"value={`$${paidInvoices.reduce((s, i) => s + i.amount, 0).toLocaleString()}`}
          trend="up"trendValue="9.3%"iconBg="#F5F3FF"iconColor="#7C3AED"icon={<svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>}
        />
        <KpiCard
      ...
      <SectionWrapper title="Quarterly Revenue"description="Q1 / Q2 revenue breakdown">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            <div
              key={q.label}
              className="rounded-xl border p-4 text-center space-y-1"style={{ background: `${q.color}10`, borderColor: `${q.color}30` }}
            >
      ...
      <section>
        <div
          className="rounded-xl border p-5 space-y-4"style={{ background: "linear-gradient(135deg, #ECFDF5 0%, #EFF6FF 100%)", borderColor: "#A7F3D0"}}
        >
          <div className="flex items-center justify-between gap-4">
            ...
            <button
              ...
              className="text-xs font-semibold px-3 py-1.5 rounded-lg border"style={{ color: "#059669", borderColor: "#A7F3D0", background: "#fff"}}
            >
      ...
      <SectionWrapper title="Billing Holds"description="Active billing holds by reason">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3">
          {[
            ...
            <div
              key={h.reason}
              className="rounded-xl border p-4 text-center space-y-2"style={{ background: h.bg, borderColor: h.border }}
            >
              <span
                className="inline-flex items-center justify-center w-8 h-8 rounded-lg text-xs font-bold mx-auto"style={{ background: h.color + "20", color: h.color }}
              >
