...
type ActivationStatus =
  | "Ready for Onboarding"| "Pending Activation"| "Onboarding Pending"| "Activation In Progress"| "Blocked"| "Pushed to AM";
type DeptActivationStatus =
  | "Department Activation Pending"| "Activation In Progress"| "Active"| "Not Started";
...
function WorkflowBanner() {
  return (
    ...
    >
      <p
        className="text-[11px] font-bold uppercase tracking-widest mb-3"style={{ color: "#7C3AED"}}
      >
        Activation Workflow
      </p>
      <div className="flex flex-wrap items-center gap-2">
        {FLOW_STEPS.map((s, i) => (
          <React.Fragment key={s.label}>
            <span
              className="text-xs font-semibold px-3 py-1.5 rounded-lg border"style={{ background: s.bg, color: s.color, borderColor: s.border }}
            >
              {s.label}
            </span>
...
function BadgeLegend() {
  return (
    <div className="flex flex-wrap gap-2 items-center">
      <span
        className="text-[11px] font-bold uppercase tracking-widest mr-1"style={{ color: "var(--rtm-text-muted)"}}
      >
        Status Key:
      </span>
      {STATUS_BADGES.map((b) => (
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
      {cards.map((k) => (
        <div
          key={k.label}
          className="rounded-xl border p-4 flex flex-col gap-1"style={{ background: k.bg, borderColor: k.border }}
        >
          <span
            className="text-[10px] font-bold uppercase tracking-wide leading-tight"style={{ color: k.color }}
          >
            {k.label}
          </span>
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-3">
        <div
          className="flex-1 h-2 rounded-full overflow-hidden"style={{ background: "var(--rtm-border-light)"}}
        >
          <div
          ...
          <tbody>
            {items.map((item, idx) => (
              <tr
                ...
                style={{
                  background:
                    item.status === "Complete"? "#F0FDF4": item.status === "Blocked"? "#FEF2F2": "var(--rtm-bg)",
                }}
              >
                <Td>
                  <span
                    className="font-medium"style={{
                      color:
                        item.status === "Complete"? "#065F46": item.status === "Blocked"? "#991B1B": "var(--rtm-text-primary)",
                    }}
                  >
                ...
                <Td>
                  ...
                  {item.status === "Complete"&& (
                    <span
                      className="text-xs font-semibold"style={{ color: "#059669"}}
                    >
                      Done
                    </span>
                  )}
...
function ClientDetailPanel({
  ...
  const [activeTab, setActiveTab] = useState<
    "summary"| "checklist"| "events">("summary");
  ...
  return (
    <div
      className="rounded-xl border mt-0 mb-2"style={{ background: "#F8FAFF", borderColor: "#BFDBFE"}}
    >
      {/* Panel header */}
      <div
        className="flex items-center justify-between px-5 py-3 border-b rounded-t-xl"style={{ background: "#EFF6FF", borderColor: "#BFDBFE"}}
      >
        <div>
          <span
            className="text-[11px] font-bold uppercase tracking-widest"style={{ color: "#1B4FD8"}}
          >
            Client Detail
          </span>
      ...
      {/* Tabs */}
      <div
        className="flex gap-0 border-b"style={{ borderColor: "#BFDBFE"}}
      >
        {tabs.map((t) => (
          <button
            key={t.id}
            onClick={() => setActiveTab(t.id)}
            className="text-xs font-semibold px-4 py-2.5 border-b-2 transition-colors"style={{
        ...
        {activeTab === "summary"&& (
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {/* Billing Summary */}
              <div
                className="rounded-lg border p-4 space-y-2"style={{ background: "var(--rtm-surface)", borderColor: "var(--rtm-border)"}}
              >
                <p
            ...
            {/* Services purchased */}
            <div
              className="rounded-lg border p-4"style={{ background: "var(--rtm-surface)", borderColor: "var(--rtm-border)"}}
            >
              <p
                className="text-[11px] font-bold uppercase tracking-widest mb-2"style={{ color: "var(--rtm-text-muted)"}}
              >
                Services Purchased
              </p>
              <div className="flex flex-wrap gap-2">
                {client.servicesPurchased.map((svc) => (
                  <span
                    key={svc}
                    className="text-xs font-semibold px-2.5 py-1 rounded-full border"style={{ background: "#EFF6FF", color: "#1B4FD8", borderColor: "#BFDBFE"}}
                  >
                    {svc}
                  </span>
                ))}
            ...
            {client.notes && (
              <div
                className="rounded-lg border p-4"style={{ background: "#FFFBEB", borderColor: "#FDE68A"}}
              >
                <p
                  className="text-[11px] font-bold uppercase tracking-widest mb-1"style={{ color: "#92400E"}}
                >
                  Notes
                </p>
        ...
        {activeTab === "events"&& (
          <div className="space-y-2">
            {client.recentEvents.map((ev, i) => (
              <div
                key={i}
                className="flex items-start gap-3 rounded-lg border px-4 py-2.5"style={{
                  background: "var(--rtm-surface)",
                  borderColor: "var(--rtm-border-light)",
                }}
              >
                <span
                  className="text-[11px] font-bold w-10 flex-shrink-0 pt-0.5"style={{ color: "var(--rtm-text-muted)"}}
                >
                  {ev.date}
                </span>
                ...
                <span
                  className="text-[11px] font-semibold flex-shrink-0"style={{ color: "var(--rtm-text-muted)"}}
                >
                  {ev.actor}
                </span>
              </div>
      ...
      {/* Row action bar */}
      <div
        className="flex flex-wrap gap-2 px-5 py-3 border-t rounded-b-xl"style={{ background: "#F0F9FF", borderColor: "#BAE6FD"}}
      >
        <ActionBtn
          label="View Client"href="/clients"variant="secondary"/>
        {client.assignedAM === "Unassigned"&& (
          <ActionBtn
            label="Assign AM"variant="primary"onClick={() => onLog(`Assign AM triggered: ${client.clientName}`)}
          />
        )}
        {(client.activationStatus === "Ready for Onboarding"||
          client.activationStatus === "Pending Activation") && (
          <ActionBtn
            label="Start Onboarding"variant="primary"onClick={() =>
              onLog(`Start Onboarding → Onboarding Pending: ${client.clientName}`)
            }
          />
        )}
        <ActionBtn
          label="Create Activation Tasks"variant="secondary"onClick={() =>
            onLog(`Activation tasks created: ${client.clientName}`)
          }
        />
        <ActionBtn
          label="Push to Account Management"href="/account-management/onboarding"variant="primary"/>
        <ActionBtn
          label="Push to Dept Activation"href="/departments/activation"variant="secondary"/>
        <ActionBtn
          label="Add Note"variant="secondary"onClick={() => onLog(`Note added: ${client.clientName}`)}
        />
        {client.activationStatus !== "Blocked"&& (
          <ActionBtn
            label="Mark Blocked"variant="danger"onClick={() => onLog(`Marked Blocked: ${client.clientName}`)}
          />
        )}
      </div>
    </div>
...
function Row({
  ...
  return (
    <div className="flex items-center gap-2">
      <span
        className="text-xs w-36 flex-shrink-0"style={{ color: "var(--rtm-text-muted)"}}
      >
        {label}
      </span>
...
function ClientsReadyForOnboarding() {
  ...
  return (
    <SectionWrapper
      title="Clients Ready for Onboarding"description="Paid clients cleared through billing whose invoice is paid, billing is cleared, and activation status qualifies for onboarding. Click any row to view client detail, readiness checklist, and take action.">
      {/* Badge legend */}
      <BadgeLegend />

      {/* Table */}
      <div
        className="mt-4 overflow-x-auto rounded-lg border"style={{ borderColor: "var(--rtm-border-light)"}}
      >
                ...
                <React.Fragment key={client.id}>
                  <tr
                    ...
                    className="cursor-pointer transition-colors"style={{
                      background: isExpanded
                        ? "#EFF6FF": client.activationStatus === "Blocked"? "#FEF9F9": "var(--rtm-bg)",
                    }}
                  >
                    {/* Client Name */}
                    <Td>
                      <div className="flex items-center gap-2">
                        <span
                          className="text-base"style={{ color: "#1B4FD8"}}
                        >
                          {isExpanded ? "": ""}
                        </span>
                        <span
                    ...
                    <Td>
                      {client.assignedAM === "Unassigned"? (
                        ...
                        <span
                          className="text-sm font-medium"style={{ color: "var(--rtm-text-primary)"}}
                        >
                          {client.assignedAM}
                        </span>
                      )}
                    ...
                    <Td wrap>
                      <span
                        className="text-xs leading-snug block max-w-[160px]"style={{ color: "var(--rtm-text-secondary)", whiteSpace: "normal"}}
                      >
                    ...
                    <Td wrap>
                      <span
                        className="text-xs leading-snug block max-w-[180px]"style={{
                          color:
                            client.activationStatus === "Blocked"? "#DC2626": "var(--rtm-text-secondary)",
                          whiteSpace: "normal",
                        }}
                      >
                    ...
                    <Td>
                      <div
                        className="flex gap-1 flex-wrap"onClick={(e) => e.stopPropagation()}
                      >
                        <ActionBtn
                          label="View"variant="ghost"href="/clients"/>
                        {client.assignedAM === "Unassigned"&& (
                          <ActionBtn
                            label="Assign AM"variant="primary"onClick={() =>
                        ...
                        {(client.activationStatus === "Ready for Onboarding"||
                          client.activationStatus === "Pending Activation") && (
                          <ActionBtn
                            label="Start Onboarding"variant="secondary"onClick={() =>
                              log(
                        ...
                        {client.activationStatus === "Blocked"&& (
                          <ActionBtn
                            label="Unblock"variant="danger"onClick={() =>
                        ...
                        {client.activationStatus !== "Pushed to AM"&& (
                          <ActionBtn
                            label="Push to AM"variant="secondary"href="/account-management/onboarding"/>
                        )}
                      </div>
                    </Td>
                  </tr>
                  ...
                  {isExpanded && (
                    <tr key={`${client.id}-detail`}>
                      <td
                        colSpan={12}
                        className="p-0"style={{ borderBottom: "2px solid #BFDBFE"}}
                      >
                        <ClientDetailPanel
                          client={client}
      ...
      {actionLog.length > 0 && (
        <div
          className="mt-4 rounded-lg border p-3"style={{
...
export default function BillingActivationPage() {
  return (
    <div className="space-y-8">
      ...
      <TaskAccessCard
        context="Activation"variant="banner"counters={{ open: 14, overdue: 2, dueToday: 6, completed: 31 }}
        createLabel="Create Activation Task"examples={["Assign AM", "Schedule Kickoff", "Create Dept Tasks", "Activation Review"]}
      />
      {/*  Header  */}
      <div>
        <p
      ...
      {/*  KPI Summary  */}
      <SectionWrapper
        title="Activation Summary"description="Real-time snapshot of the onboarding queue">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Link
          href="/account-management/onboarding"className="rtm-btn-primary text-sm inline-flex items-center gap-1">
          AM Onboarding →
        </Link>
        <Link
          href="/departments/activation"className="rtm-btn-secondary text-sm inline-flex items-center gap-1">
          Dept Activation →
        </Link>
        <Link
