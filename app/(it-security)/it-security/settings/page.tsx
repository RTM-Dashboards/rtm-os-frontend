...
const teamMembers = [
  { name: "Nathan S.", role: "Department Head", access: "Full Access", status: "Active", clients: 10, lastActive: "Today, 7:50 AM"},
  { name: "Yara E.", role: "Sr. Security Engineer", access: "Edit", status: "Active", clients: 7, lastActive: "Today, 8:10 AM"},
  { name: "Caden P.", role: "IT Support Specialist", access: "Edit", status: "Active", clients: 5, lastActive: "Yesterday, 7:00 PM"},
  { name: "Renata B.", role: "Compliance Analyst", access: "Edit", status: "Active", clients: 4, lastActive: "Today, 9:50 AM"},
  { name: "Hugo F.", role: "IT Coordinator", access: "View", status: "Inactive", clients: 0, lastActive: "2 weeks ago"},
];

const workspacePermissions = [
  { label: "Manage System Access", enabled: true },
  { label: "Run Security Audits", enabled: true },
  { label: "View Audit Logs", enabled: true },
  { label: "Revoke User Access", enabled: false },
  { label: "Deploy Security Patches", enabled: false },
  { label: "Export Compliance Reports", enabled: true },
];

const notificationPrefs = [
  { label: "Unauthorized login attempt", channel: "Email + Slack", enabled: true },
  { label: "Security audit completed", channel: "Slack", enabled: true },
  { label: "Critical vulnerability found", channel: "Email + Slack", enabled: true },
  { label: "SSL certificate expiring in 30 days", channel: "Email", enabled: true },
  { label: "Patch deployment required", channel: "Email", enabled: false },
];

const defaultTaskSettings = [
  { label: "Default Priority", value: "Critical"},
  { label: "Default Assignee", value: "Security Engineer"},
  { label: "Incident Response SLA", value: "Within 2 hours"},
  { label: "Audit Cycle", value: "Quarterly"},
  { label: "Task Template", value: "IT Security Standard"},
];
...
export default function ITSecuritySettingsPage() {
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border p-5"style={{ background: "var(--rtm-surface)", borderColor: "var(--rtm-border)"}}>
        ...
        <div className="flex items-center gap-4 p-4 rounded-lg"style={{ background: "var(--rtm-bg)", border: "1px solid var(--rtm-border-light)"}}>
          <div className="w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-lg flex-shrink-0"style={{ background: "var(--rtm-blue)"}}>N</div>
          <div className="flex-1">
            <p className="font-bold text-sm"style={{ color: "var(--rtm-text-primary)"}}>Nathan S.</p>
            <p className="text-xs mt-0.5"style={{ color: "var(--rtm-text-secondary)"}}>nathan@rtm.io · IT & Security Department Head</p>
          </div>
      ...
      <KpiSettingsSection departmentSlug="it-security" />
    </div>
  );
}
