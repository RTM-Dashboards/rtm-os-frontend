...
function DepartmentDrawer({ dept, onClose, activeTab, setActiveTab }: DrawerProps) {
  return (
    <div className="fixed inset-0 z-50 flex justify-end" style={{ background: "rgba(15,28,56,0.35)" }} onClick={onClose}>
      <div
        className="relative flex flex-col w-full max-w-2xl h-full overflow-hidden"
...
function RowActions({ dept, onView }: { dept: Department; onView: () => void }) {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="px-2.5 py-1 rounded-lg text-xs font-semibold border transition-colors hover:bg-slate-50"
        style={{ borderColor: "var(--rtm-border)", color: "var(--rtm-text-secondary)" }}
      >
        Actions ▾
      </button>
      {open && (
        <>
          ...
          <div
            className="absolute right-0 z-30 mt-1 w-52 rounded-xl border py-1 shadow-lg"
            style={{ background: "var(--rtm-surface)", borderColor: "var(--rtm-border)" }}
          >
...
export default function DepartmentsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {deptTypes.map((type) => {
          ...
          return (
            <div
              key={type}
              className="rounded-xl border p-4 cursor-pointer transition-colors hover:shadow-sm"
      ...
      {/* ── Filters ── */}
      <div
        className="rounded-xl border px-5 py-4 flex flex-wrap items-center gap-3"
        style={{ background: "var(--rtm-surface)", borderColor: "var(--rtm-border)" }}
      >
        <input
          type="text"
          ...
          className="px-3 py-2 rounded-lg border text-sm outline-none focus:ring-2 flex-1 min-w-[200px]"
        ...
        {(typeFilter !== "All" || statusFilter !== "All" || search !== "") && (
          <button
            ...
            className="px-3 py-2 rounded-lg border text-xs font-semibold hover:bg-slate-50 transition-colors"
            style={{ borderColor: "var(--rtm-border)", color: "var(--rtm-text-secondary)" }}
          >
            Clear Filters
          </button>
            ...
            <tbody>
              {filtered.map((dept, i) => (
                <tr
                  ...
                  style={{ borderColor: "var(--rtm-border)", background: i % 2 === 1 ? "transparent" : undefined }}
