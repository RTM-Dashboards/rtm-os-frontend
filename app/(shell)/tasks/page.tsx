...
function ProjectDetailDrawer({ project, onClose }: { project: Project; onClose: () => void }) {
  const [activeTab, setActiveTab] = useState<
    "overview" | "contract" | "lineitems" | "milestones" | "tasklists" | "tasks" | "departments" | "dependencies" | "standards" | "blockers" | "timeline" | "notes" | "executive"
  >("overview");
              ...
              {project.departments.map((dept) => {
                ...
                return (
                  <div key={dept.department} className="rounded-xl p-4" style={{ background: "var(--rtm-bg)", border: "1px solid var(--rtm-border)" }}>
                    ...
                    {dept.delayReason && (
                      <div className="text-xs mt-1 p-2 rounded-lg" style={{ background: "#FFFBEB", color: "#D97706" }}>⚠️ {dept.delayReason}</div>
                    )}
                  </div>
                );
              })}
            ...
            <div className="flex flex-col gap-3">
              ...
              {allDeps.map((dep) => (
                <div key={dep.id} className="flex items-center gap-3 p-3 rounded-lg"
                  style={{ background: dep.direction === "blocked-by" ? "#FEF2F2" : "#FFFBEB", border: `1px solid ${dep.direction === "blocked-by" ? "#FECACA" : "#FDE68A"}` }}>
                  <span className="text-sm">{dep.direction === "blocked-by" ? "🚫" : "⚠️"}</span>
...
{m.label}
      ...
      <div className="flex-1 px-6 py-5 max-w-[1600px] mx-auto w-full">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 xl:grid-cols-8 gap-3 mb-6">
          <KpiCard label="Total Projects" value={kpis.totalProjects} icon="🏗️" color="var(--rtm-blue)" sub="All projects" />
          <KpiCard label="Active" value={kpis.activeProjects} icon="⚡" color="#D97706" sub="In Progress / Launched" />
          <KpiCard label="Blocked / Pending" value={kpis.blockedProjects} icon="🚫" color="#DC2626" sub="Needs attention" />
          <KpiCard label="Completed" value={kpis.completedProjects} icon="✅" color="#059669" sub="Delivered" />
          <KpiCard label="Total Tasks" value={kpis.totalTasks} icon="📋" color="var(--rtm-blue)" sub="Across all projects" />
          <KpiCard label="Open Tasks" value={kpis.openTasks} icon="📂" color="#D97706" sub="Active work" />
          <KpiCard label="Blocked Tasks" value={kpis.blockedTasks} icon="🔴" color="#DC2626" sub="Need unblocking" />
          <KpiCard label="Open Blockers" value={kpis.totalBlockers} icon="🛑" color="#7C3AED" sub="Project blockers" />
        </div>
