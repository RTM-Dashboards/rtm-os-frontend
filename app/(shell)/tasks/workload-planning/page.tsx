...
function SLABar({ compliance, color }: { compliance: number; color: string }) {
  ...
  return (
    <div className="w-full rounded-full h-2 overflow-hidden"style={{ background: "#E5E7EB"}}>
      <div
        className="h-2 rounded-full transition-all"style={{ width: `${compliance}%`, background: barColor }}
      />
    </div>
  );
}
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Link
            href="/tasks/templates"className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold border"style={{ borderColor: "var(--rtm-border)", color: "var(--rtm-text-primary)", background: "var(--rtm-surface)"}}
          >
             View Templates
          </Link>
          <button
              ...
              <div key={dept.dept}>
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-3">
                    <span
                      className="text-[11px] font-bold px-2.5 py-0.5 rounded-full"style={{ background: dept.bg, color: dept.color, border: `1px solid ${dept.border}` }}
                    >
                      {dept.dept}
                    </span>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        {DEPARTMENT_THROUGHPUT.map((dept) => (
          <div
            key={dept.dept}
            className="rounded-xl p-4 text-center"style={{ background: dept.bg, border: `1px solid ${dept.border}` }}
          >
      ...
      {/* Info banner */}
      <div
        className="rounded-xl p-5"style={{ background: "#EFF6FF", border: "1px solid #BFDBFE"}}
      >
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <div className="flex gap-2 mt-3">
              <Link
                href="/tasks/templates"className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-bold"style={{ background: "#1D4ED8", color: "#fff"}}
              >
                 Manage Templates →
              </Link>
              <Link
                href="/tasks/activation-rules"className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold border"style={{ borderColor: "#BFDBFE", color: "#1D4ED8"}}
              >
                 Activation Rules
              </Link>
            </div>
