...
const MODULE_LINKS = [
  ...
  { label: "Task Templates",       href: "/tasks/templates",             icon: ""},
...
const PLANNED_METRICS = [
  ...
  { label: "Avg Time to Complete",      icon: "⏱", desc: "Average days from task creation to completion by department"},
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Link
            href="/tasks"className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold border"style={{ borderColor: "var(--rtm-border)", color: "var(--rtm-text-primary)", background: "var(--rtm-surface)"}}
          >
            ← Back to Tasks
          </Link>
        </div>
      ...
      {/* Coming Soon Banner */}
      <div className="rounded-xl p-8 flex flex-col items-center justify-center text-center gap-4"style={{ background: "var(--rtm-blue-xlight)", border: "1px solid #BFDBFE"}}>
        ...
        <div className="flex flex-wrap gap-2 justify-center mt-2">
          <Link
            href="/tasks"className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-bold text-white"style={{ background: "var(--rtm-blue)"}}
          >
             Task Management
          </Link>
          <Link
            href="/tasks/workload-planning"className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold border"style={{ borderColor: "#BFDBFE", color: "#1E40AF", background: "white"}}
          >
             Dept. Throughput
          </Link>
        </div>
      ...
      <div className="rounded-xl overflow-hidden"style={{ background: "var(--rtm-surface)", border: "1px solid var(--rtm-border)"}}>
        ...
        <div className="p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {PLANNED_METRICS.map((metric) => (
            <div
              key={metric.label}
              className="rounded-xl p-4 flex flex-col gap-2"style={{ background: "var(--rtm-bg)", border: "1px solid var(--rtm-border)"}}
            >
              ...
              <div
                className="text-[10px] font-bold px-2 py-0.5 rounded-full self-start"style={{ background: "var(--rtm-blue-light)", color: "var(--rtm-blue)"}}
              >
                Coming soon
              </div>
            </div>
        ...
        <div className="p-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {MODULE_LINKS.map((module) => (
            <Link
              key={module.href}
              href={module.href}
              className="rounded-xl p-4 flex flex-col gap-2 transition-opacity hover:opacity-80"style={{ background: "var(--rtm-blue-xlight)", border: "1px solid #BFDBFE"}}
            >
