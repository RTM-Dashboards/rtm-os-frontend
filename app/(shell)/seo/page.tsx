...
const columns: Column<SeoClient>[] = [
  ...
  { key: "topKeyword", header: "Top Keyword" },
  {
    ...
    render: (value) => {
      ...
      const color = v <= 3 ? "text-emerald-600 dark:text-emerald-400" : v <= 7 ? "text-amber-600 dark:text-amber-400" : "text-slate-600";
      return <span className={`font-bold text-sm ${color}`}>#{v}</span>;
    },
  },
  { key: "monthlyImpressions", header: "Impressions", width: "120px" },
  {
    key: "domainAuthority", header: "DA", width: "120px",
...
const alerts: AlertItem[] = [
  ...
  { id: "2", severity: "info",    title: "4 clients in top-3 positions",           description: "Apex Roofing, Pacific Dental, Summit Landscaping, and Blue Ridge all holding top spots.", action: "View" },
];
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">SEO</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Search rankings, keyword strategy & domain authority.</p>
        </div>
