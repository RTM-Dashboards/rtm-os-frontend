...
const teamMembers = [
  { name: "Sandra V.", role: "Department Head", access: "Full Access", status: "Active", clients: 18, lastActive: "Today, 9:15 AM"},
  { name: "Will C.", role: "Sr. Data Analyst", access: "Edit", status: "Active", clients: 12, lastActive: "Today, 8:50 AM"},
  { name: "Naomi A.", role: "Reporting Specialist", access: "Edit", status: "Active", clients: 8, lastActive: "Yesterday, 4:30 PM"},
  { name: "Felix R.", role: "BI Developer", access: "Edit", status: "Active", clients: 7, lastActive: "Today, 10:20 AM"},
  { name: "Grace M.", role: "Report Coordinator", access: "View", status: "Inactive", clients: 0, lastActive: "1 week ago"},
];

const workspacePermissions = [
  { label: "Generate Client Reports", enabled: true },
  { label: "Schedule Auto-Reports", enabled: true },
  { label: "Export Raw Data", enabled: true },
  { label: "Edit Report Templates", enabled: false },
  { label: "Share Reports Externally", enabled: true },
  { label: "Access All Department Data", enabled: false },
];

const notificationPrefs = [
  { label: "Monthly report cycle begins", channel: "Slack", enabled: true },
  { label: "Report overdue for client", channel: "Email + Slack", enabled: true },
  { label: "Data source disconnected", channel: "Email + Slack", enabled: true },
  { label: "Report viewed by client", channel: "Slack", enabled: true },
  { label: "Anomaly detected in data", channel: "Email", enabled: false },
];

const defaultTaskSettings = [
  { label: "Default Priority", value: "Medium"},
  { label: "Default Assignee", value: "Reporting Specialist"},
  { label: "Report Delivery Day", value: "1st of each month"},
  { label: "Data Refresh Cycle", value: "Weekly"},
  { label: "Task Template", value: "Reporting Standard"},
];
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border p-5"style={{ background: "var(--rtm-surface)", borderColor: "var(--rtm-border)"}}>
        ...
        <div className="flex items-center gap-4 p-4 rounded-lg"style={{ background: "var(--rtm-bg)", border: "1px solid var(--rtm-border-light)"}}>
          <div className="w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-lg flex-shrink-0"style={{ background: "var(--rtm-blue)"}}>S</div>
          <div className="flex-1">
            <p className="font-bold text-sm"style={{ color: "var(--rtm-text-primary)"}}>Sandra V.</p>
            <p className="text-xs mt-0.5"style={{ color: "var(--rtm-text-secondary)"}}>sandra@rtm.io · Reporting Department Head</p>
          </div>
      ...
      <KpiSettingsSection departmentSlug="reporting" />
    </div>
  );
}
