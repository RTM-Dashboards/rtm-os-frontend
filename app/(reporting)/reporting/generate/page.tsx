              ...
              {(["configure", "preview", "generate"] as GenerateStep[]).map((s, i) => (
                <div key={s} className="flex items-center gap-2">
                  <button onClick={() => { if (step !== "generate"|| !generated) setStep(s); }}
                    className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all"style={{ background: step === s ? "var(--rtm-blue)": "var(--rtm-surface)", color: step === s ? "white": "var(--rtm-text-secondary)", border: "1px solid var(--rtm-border)"}}>
