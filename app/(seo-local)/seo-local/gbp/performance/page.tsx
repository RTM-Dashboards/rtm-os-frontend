...
const googlePostsColumns: Column<GooglePostRow>[] = [
  ...
  { key: "imageAttached", header: "Image", width: "70px",
    render: (v) =>
      v ? <span className="text-emerald-500 text-xs font-bold">Yes</span> : <span className="text-slate-300 text-sm">—</span> },
...
export default function GbpPerformancePage() {
  ...
  const quickActions: QuickAction[] = [
    { label: "Optimize GBP Profile", description: "Fill missing fields",   icon: "", color: "bg-blue-100 text-blue-600",   disabled: true, disabledReason: "Not yet available" },
    { label: "Review Request",        description: "Send review links",     icon: "", color: "bg-amber-100 text-amber-600",  disabled: true, disabledReason: "Not yet available" },
    { label: "Q&A Sweep",             description: "Answer open questions",         color: "bg-purple-100 text-purple-600", disabled: true, disabledReason: "Not yet available" },
    { label: "Photo Upload",          description: "Add fresh photos",      icon: "", color: "bg-emerald-100 text-emerald-600", disabled: true, disabledReason: "Not yet available" },
    { label: "Post Content",          description: "GBP weekly post",       icon: "", color: "bg-slate-100 text-slate-600",  disabled: true, disabledReason: "Not yet available" },
    { label: "Visibility Report",     description: "View GBP reports",      icon: "", color: "bg-red-100 text-red-600",      onClick: () => router.push("/seo-local/gbp/reports") },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {isEnabled("seo-gbp-calls") && <KpiCard
        ...
        />}
        {isEnabled("seo-gbp-calls") && <KpiCard
          title="Website Clicks"value={totals.clicks.toLocaleString()}
          trend="up"trendValue="+21%"trendLabel={compLabel ?? "vs last period"}
          iconBg="#F5F3FF"iconColor="#7C3AED"icon={<svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5M7.188 2.239l.777 2.897M5.136 7.965l-2.898-.777M13.95 4.05l-2.122 2.122m-5.657 5.656l-2.12 2.122"/></svg>}
        />}
        {isEnabled("seo-gbp-direction-requests") && <KpiCard
          title="Direction Requests"value={totals.directions.toString()}
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {isEnabled("seo-gbp-calls") && <KpiCard
          title="GBP Leads"value={totals.leads.toString()}
        ...
        />}
        {isEnabled("seo-local-visibility-score") && <KpiCard
          title="Avg Local Visibility Score"value={`${totals.avgVisibility}/100`}
        ...
        />}
        {isEnabled("seo-review-velocity") && <KpiCard
          title="Review Growth (Total)"value={`+${totals.reviewGrowth}`}
          trend="up"trendValue="+11 MoM"trendLabel={compLabel ?? "vs last period"}
      ...
      <SectionWrapper
        title="Location GBP Performance"description={`${filteredLocations.length} location${filteredLocations.length !== 1 ? "s": ""} — ${DATE_RANGE_LABELS[filters.dateRange]}`}
        noPadding
        actions={
      ...
      <div className="pt-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
          {isEnabled("seo-gbp-views") && <KpiCard
            title="Total Profile Views"value={profileViewsTotals.totalViews.toLocaleString()}
            trend="up"trendValue="+18%"trendLabel={compLabel ?? "vs last period"}
            iconBg="var(--rtm-blue-light)"iconColor="var(--rtm-blue)"icon={<svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>}
          />}
          {isEnabled("seo-gbp-views") && <KpiCard
            title="Search Views"value={profileViewsTotals.searchViews.toLocaleString()}
            trend="up"trendValue="+15%"trendLabel={compLabel ?? "vs last period"}
            iconBg="#F5F3FF"iconColor="#7C3AED"icon={<svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>}
          />}
          {isEnabled("seo-gbp-views") && <KpiCard
            title="Maps Views"value={profileViewsTotals.mapsViews.toLocaleString()}
          ...
          />}
          {isEnabled("seo-gbp-calls") && <KpiCard
            title="Website Clicks"value={profileViewsTotals.websiteClicks.toLocaleString()}
            trend="up"trendValue="+21%"trendLabel={compLabel ?? "vs last period"}
            iconBg="#ECFDF5"iconColor="#059669"icon={<svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>}
          />}
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
          {isEnabled("seo-gbp-calls") && <KpiCard
            title="Calls"value={profileViewsTotals.calls.toString()}
            trend="up"trendValue="+14%"trendLabel={compLabel ?? "vs last period"}
            iconBg="#ECFDF5"iconColor="#059669"icon={<svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>}
          />}
          {isEnabled("seo-gbp-direction-requests") && <KpiCard
            title="Direction Requests"value={profileViewsTotals.directions.toString()}
            trend="up"trendValue="+7%"trendLabel={compLabel ?? "vs last period"}
            iconBg="#FFFBEB"iconColor="#D97706"icon={<svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>}
          />}
          {isEnabled("seo-gbp-calls") && <KpiCard
            title="Form Submissions"value={profileViewsTotals.forms.toString()}
            trend="up"trendValue="+9%"trendLabel={compLabel ?? "vs last period"}
            iconBg="var(--rtm-blue-light)"iconColor="var(--rtm-blue)"icon={<svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>}
          />}
          {isEnabled("seo-gbp-calls") && <KpiCard
            title="Booked Leads"value={profileViewsTotals.booked.toString()}
            trend="up"trendValue="+19%"trendLabel={compLabel ?? "vs last period"}
            iconBg="#ECFDF5"iconColor="#059669"icon={<svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>}
          />}
        </div>
        ...
        {/* Profile Views Table */}
        <SectionWrapper
          title="Profile Views by Location"description={`${filteredProfileViews.length} location${filteredProfileViews.length !== 1 ? "s": ""} — ${DATE_RANGE_LABELS[filters.dateRange]}`}
          noPadding
          actions={
            <span className="text-xs"style={{ color: "var(--rtm-text-muted)"}}>Mock data</span>
          }
      ...
      <div className="pt-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          <KpiCard
            title="Posts Published"value={postKpis.published.toString()}
            trend="up"trendValue="+2"trendLabel={compLabel ?? "vs last period"}
          ...
          />
          <KpiCard
            title="Scheduled Posts"value={postKpis.scheduled.toString()}
            trend="up"trendValue="+1"trendLabel={compLabel ?? "vs last period"}
            iconBg="var(--rtm-blue-light)"iconColor="var(--rtm-blue)"icon={<svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>}
          />
          <KpiCard
            title="Needs Approval"value={postKpis.needsApproval.toString()}
            trend={postKpis.needsApproval > 0 ? "down": "neutral"}
            trendValue={postKpis.needsApproval > 0 ? "Action needed": "All clear"}
            trendLabel=""iconBg="#FFFBEB"iconColor="#D97706"icon={<svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>}
          />
          <KpiCard
            title="Expired Posts"value={postKpis.expired.toString()}
            trend="down"trendValue="Needs refresh"trendLabel=""iconBg="#FEF2F2"iconColor="#DC2626"icon={<svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>}
          />
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
          <KpiCard
            title="Total Post Views"value={postKpis.totalViews.toLocaleString()}
            trend="up"trendValue="+24%"trendLabel={compLabel ?? "vs last period"}
            iconBg="var(--rtm-blue-light)"iconColor="var(--rtm-blue)"icon={<svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>}
          />
          <KpiCard
            title="Total Post Clicks"value={postKpis.totalClicks.toLocaleString()}
            trend="up"trendValue="+18%"trendLabel={compLabel ?? "vs last period"}
            iconBg="#ECFDF5"iconColor="#059669"icon={<svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5"/></svg>}
          />
          <KpiCard
            title="Avg Engagement Rate"value={`${postKpis.avgEngRate.toFixed(1)}%`}
            trend="up"trendValue="+0.8%"trendLabel={compLabel ?? "vs last period"}
            iconBg="#F5F3FF"iconColor="#7C3AED"icon={<svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/></svg>}
          />
          <div
            className="rounded-xl border p-4 flex flex-col gap-1"style={{ background: "var(--rtm-surface)", borderColor: "var(--rtm-border)"}}
          >
        ...
        {/* Google Posts Table */}
        <SectionWrapper
          title="Google Posts by Location"description={`${allGooglePosts.length} posts — ${DATE_RANGE_LABELS[filters.dateRange]}`}
          noPadding
          actions={
       ...
       */}
      <div
        className="rounded-2xl border-2 border-dashed p-6"style={{ borderColor: "var(--rtm-border)", background: "var(--rtm-surface-subtle, var(--rtm-surface))"}}
      >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <button
              key={action.label}
              disabled
              title="Coming Soon — Mock UI"className={`flex flex-col items-center justify-center gap-2 rounded-xl border px-3 py-4 text-sm font-semibold cursor-not-allowed opacity-70 transition-colors ${action.color}`}
            >
