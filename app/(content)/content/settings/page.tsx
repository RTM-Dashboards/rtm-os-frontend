...
const teamMembers = [
  ...
  { name: "Marco D.", role: "Sr. Content Strategist", access: "Edit", status: "Active", clients: 8, lastActive: "Today, 9:10 AM"},
  { name: "Nia C.", role: "Content Writer", access: "Edit", status: "Active", clients: 5, lastActive: "Yesterday, 5:00 PM"},
  { name: "James P.", role: "Video Editor", access: "Edit", status: "Active", clients: 4, lastActive: "Today, 7:50 AM"},
  { name: "Sophie R.", role: "Content Coordinator", access: "View", status: "Inactive", clients: 0, lastActive: "4 days ago"},
];

const workspacePermissions = [
  { label: "Create & Publish Content", enabled: true },
  { label: "Assign Content Tasks", enabled: true },
  { label: "Manage Content Calendar", enabled: true },
  { label: "Approve Client Content", enabled: false },
  { label: "Export Content Reports", enabled: true },
  { label: "Archive Content", enabled: false },
];

const notificationPrefs = [
  { label: "Content piece due today", channel: "Slack", enabled: true },
  { label: "Client approval required", channel: "Email + Slack", enabled: true },
  { label: "Content queue empty", channel: "Slack", enabled: true },
  { label: "Campaign deadline in 48h", channel: "Email", enabled: true },
  { label: "Revision requested", channel: "Email + Slack", enabled: false },
];

const defaultTaskSettings = [
  { label: "Default Priority", value: "Medium"},
  { label: "Default Assignee", value: "Content Writer"},
  { label: "Draft Due Offset", value: "5 business days"},
  { label: "Review Window", value: "2 business days"},
  { label: "Task Template", value: "Content Standard"},
];
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border p-5"style={{ background: "var(--rtm-surface)", borderColor: "var(--rtm-border)"}}>
        ...
        <div className="flex items-center gap-4 p-4 rounded-lg"style={{ background: "var(--rtm-bg)", border: "1px solid var(--rtm-border-light)"}}>
          <div className="w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-lg flex-shrink-0"style={{ background: "var(--rtm-blue)"}}>E</div>
          <div className="flex-1">
            <p className="font-bold text-sm"style={{ color: "var(--rtm-text-primary)"}}>Elena H.</p>
            <p className="text-xs mt-0.5"style={{ color: "var(--rtm-text-secondary)"}}>elena@rtm.io · Content Department Head</p>
          </div>
