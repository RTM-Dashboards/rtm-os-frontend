    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        {isEnabled("paid-cost-per-qualified") && <KpiCard
          ...
          trend="down"trendValue="-$5.20"trendLabel={compLabel ?? "vs last period"}
        ...
        />}
        {isEnabled("paid-meta-ctr") && <KpiCard
          title="Avg CTR"value={`${totals.avgCtr.toFixed(1)}%`}
          trend="up"trendValue="+0.4%"trendLabel={compLabel ?? "vs last period"}
      ...
      <div className="grid grid-cols-2 xl:grid-cols-3 gap-4">
        {isEnabled("paid-meta-cpm") && <KpiCard
          ...
          trend="down"trendValue="-$1.20"trendLabel={compLabel ?? "vs last period"}
        ...
        />}
        {isEnabled("paid-meta-frequency") && <KpiCard
        ...
        />}
        {isEnabled("paid-total-leads") && <KpiCard
          title="Campaign Health"value={`${filteredCampaigns.filter(c => c.campaignHealth === "excellent"|| c.campaignHealth === "good").length} / ${filteredCampaigns.length} Healthy`}
          trend={fatigueCounts.high === 0 ? "up": "down"}
          trendValue={fatigueCounts.high > 0 ? `${fatigueCounts.high} need refresh` : "All good"}
          trendLabel="creative status"iconBg="#ECFDF5"iconColor="#059669"icon={<svg className="w-5 h-5"fill="none"stroke="currentColor"viewBox="0 0 24 24"><path strokeLinecap="round"strokeLinejoin="round"strokeWidth={1.75} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>}
        />}
      </div>
