...
const workspacePermissions = [
  { label: "Manage Clients", enabled: true },
  { label: "Assign Tasks", enabled: true },
  { label: "Generate Reports", enabled: true },
  { label: "Edit Billing Info", enabled: false },
  { label: "Invite Team Members", enabled: false },
  { label: "Export Data", enabled: true },
];

const notificationPrefs = [
  { label: "New client assigned", channel: "Email + Slack", enabled: true },
  { label: "Client health score drops below 70", channel: "Email + Slack", enabled: true },
  { label: "Check-in due today", channel: "Slack", enabled: true },
  { label: "Report overdue", channel: "Email", enabled: true },
  { label: "Task escalated", channel: "Email", enabled: false },
];

const defaultTaskSettings = [
  { label: "Default Priority", value: "Medium"},
  { label: "Default Assignee", value: "Department Head"},
  { label: "Due Date Offset", value: "3 business days"},
  { label: "Auto-close After", value: "30 days inactive"},
  { label: "Task Template", value: "Account Management Standard"},
];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border p-5"style={{ background: "var(--rtm-surface)", borderColor: "var(--rtm-border)"}}>
        ...
        <div className="flex items-center gap-4 p-4 rounded-lg"style={{ background: "var(--rtm-bg)", border: "1px solid var(--rtm-border-light)"}}>
          ...
          <div className="flex-1">
            <p className="font-bold text-sm"style={{ color: "var(--rtm-text-primary)"}}>Jordan M.</p>
            <p className="text-xs mt-0.5"style={{ color: "var(--rtm-text-secondary)"}}>jordan@rtm.io · Account Management Department Head</p>
          </div>
          <span
            className="text-[11px] font-bold px-2.5 py-1 rounded-full"style={{ background: "#1B4FD818", color: "#1B4FD8"}}
          >
            Admin
          </span>
        </div>
      ...
      <div className="rounded-xl border overflow-hidden"style={{ background: "var(--rtm-surface)", borderColor: "var(--rtm-border)"}}>
        <div
          className="px-5 py-3 flex items-center justify-between"style={{ background: "var(--rtm-bg)", borderBottom: "1px solid var(--rtm-border)"}}
        >
          <div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottom: "1px solid var(--rtm-border-light)"}}>
                {["Member", "Role", "Access Level", "Status", "Assigned Clients", "Last Active"].map((h) => (
                  <th
                    key={h}
                    className="px-4 py-3 text-left text-xs font-semibold whitespace-nowrap"style={{ color: "var(--rtm-text-muted)"}}
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              {teamMembers.map((m, i) => (
                <tr
                  key={m.name}
                  style={{ borderBottom: i < teamMembers.length - 1 ? "1px solid var(--rtm-border-light)": "none"}}
                >
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      <div
                        className="w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0"style={{ background: "var(--rtm-blue)"}}
                      >
                        {m.name.charAt(0)}
                      </div>
                  ...
                  <td className="px-4 py-3">
                    <span
                      className="text-[11px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap"style={{ background: `${accessColors[m.access]}18`, color: accessColors[m.access] }}
                    >
                      {m.access}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className="text-[11px] font-bold px-2 py-0.5 rounded-full"style={
                        m.status === "Active"? { background: "#05966918", color: "#059669"}
      ...
      <div className="rounded-xl border p-5"style={{ background: "var(--rtm-surface)", borderColor: "var(--rtm-border)"}}>
        ...
        <div className="space-y-2">
          {permissions.map((perm, i) => (
            <div
              key={perm.label}
              className="flex items-center justify-between p-3 rounded-lg"style={{ background: "var(--rtm-bg)", border: "1px solid var(--rtm-border-light)"}}
            >
              <span className="text-sm"style={{ color: "var(--rtm-text-secondary)"}}>{perm.label}</span>
              <button
                onClick={() =>
                  setPermissions((prev) =>
                    prev.map((p, idx) => (idx === i ? { ...p, enabled: !p.enabled } : p))
                  )
                }
              ...
              >
                <span
                  className="inline-block w-3.5 h-3.5 rounded-full bg-white shadow transition-transform"style={{ transform: perm.enabled ? "translateX(18px)": "translateX(3px)"}}
                />
              </button>
            </div>
          ))}
        ...
        <div className="space-y-2">
          {notifications.map((notif, i) => (
            ...
            >
              ...
              >
                <span
                  className="inline-block w-3.5 h-3.5 rounded-full bg-white shadow transition-transform"style={{ transform: notif.enabled ? "translateX(18px)": "translateX(3px)"}}
                />
              </button>
            </div>
          ))}
