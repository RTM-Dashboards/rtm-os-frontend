  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
        <KpiCard
          label="Active Offboardings"value={active.length}
          sub="in progress"accent="#1D4ED8"borderColor="border-blue-200"bg="bg-blue-50"/>
        <KpiCard
          label="Pending Asset Transfers"value={pendingAssets.length}
          sub="not yet transferred"accent="#0369A1"borderColor="border-sky-200"bg="bg-sky-50"/>
        <KpiCard
          label="Pending Access Removals"value={pendingAccess.length}
          sub="access still active"accent="#DC2626"borderColor="border-red-200"bg="bg-red-50"/>
        <KpiCard
          label="Pending Final Billing"value={pendingBilling.length}
          sub="invoices outstanding"accent="#B45309"borderColor="border-amber-200"bg="bg-amber-50"/>
        <KpiCard
          label="Completed Offboardings"value={completed.length}
          sub="archived"accent="#059669"borderColor="border-emerald-200"bg="bg-emerald-50"/>
        <KpiCard
          label="Revenue Lost"value={fmt$(mrrLost)}
          sub={`${fmt$(arrLost)}/yr ARR`}
          accent="#475569"borderColor="border-slate-300"bg="bg-slate-50"/>
      </div>
...
function OffboardingProject({ record }: { record: OffboardingRecord }) {
  const doneItems = record.checklist.filter(
    (c) => c.status === "Completed"|| c.status === "Not Applicable").length;
            ...
            {record.activityTimeline.map((event, idx) => {
              ...
              return (
                <div key={`${event.date}-${idx}`} className="relative flex gap-5">
                  ...
                  <div
                    className="flex-1 rounded-xl border p-4"style={{ borderColor: color + "40", background: color + "08"}}
                  >
...
type DetailTab =
  | "project"| "checklist"| "dept-tasks"| "asset-transfer"| "access-removal"| "final-billing"| "knowledge"| "ai-summary"| "timeline";
